...
async function prepareFixture(fixtureName: string): Promise<string> {
  const base = await fs.mkdtemp(path.join(tmpdir(), 'clearspec-cli-e2e-'));
  tempRoots.push(base);
...
describe('clearspec CLI e2e basics', () => {
  it('shows help output', async () => {
    const result = await runCLI(['--help']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Usage: clearspec');
    expect(result.stderr).toBe('');
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('ClearSpec Setup Complete');

      // Check that skills were created for multiple tools
      const claudeSkillPath = path.join(emptyProjectDir, '.claude/skills/clearspec-explore/SKILL.md');
      const cursorSkillPath = path.join(emptyProjectDir, '.cursor/skills/clearspec-explore/SKILL.md');
