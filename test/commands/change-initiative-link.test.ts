...
describe('repo-local change initiative links', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clearspec-change-initiative-link-'));
    ...
    fs.mkdirSync(path.join(tempDir, 'clearspec', 'changes'), { recursive: true });
  });
  ...
  function changeDir(id: string): string {
    return path.join(tempDir, 'clearspec', 'changes', id);
  }

  function metadataPath(id: string): string {
    return path.join(changeDir(id), '.clearspec.yaml');
  }
  ...
  it('does not write a change when initiative lookup fails', async () => {
    ...
    expect(payload.status[0].fix).toBe('clearspec initiative list');
  ...
  it('does not write an existing change when set change initiative lookup fails', async () => {
    ...
    expect(missingPayload.status[0].fix).toBe('clearspec initiative list');
    ...
    expect(parseJson(ambiguous).status[0].fix).toBe(
      'clearspec initiative show billing-launch --store <store>'
    );
    expect(fs.readFileSync(metadataPath('set-lookup-failure'), 'utf-8')).toBe(before);
  });
