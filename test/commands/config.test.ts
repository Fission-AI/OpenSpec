...
describe('config command integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `lightspec-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('should use XDG_CONFIG_HOME for config path', async () => {
    ...
    expect(configPath).toBe(path.join(tempDir, 'lightspec', 'config.json'));
  });
...
describe('config command shell completion registry', () => {
  it('should have config command in registry', async () => {
    ...
    expect(configCmd?.description).toBe('View and modify global LightSpec configuration');
  });
