...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'pastelsdd-artifact-workflow-'));
    changesDir = path.join(tempDir, 'pastelsdd', 'changes');
  ...
  describe('status command', () => {
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('pastelsdd new change');
    });
  ...
  describe('new change command', () => {
    ...
    it('creates workspace-planning changes under the workspace root without touching linked repos', async () => {
      ...
      await fs.mkdir(path.join(api, 'pastelsdd', 'specs'), { recursive: true });
      ...
      const metadata = await fs.readFile(path.join(changeDir, '.pastelsdd.yaml'), 'utf-8');
      ...
      await expect(fs.stat(path.join(api, 'pastelsdd', 'changes'))).rejects.toMatchObject({
        code: 'ENOENT',
      });
    });
  ...
  describe('instructions apply command', () => {
    ...
    it('resolves single-star glob artifacts consistently between status and apply', async () => {
      const schemaDir = path.join(tempDir, 'pastelsdd', 'schemas', 'glob-test');
...
`
      ...
      await fs.mkdir(path.dirname(specPath), { recursive: true });
      await fs.writeFile(path.join(changeDir, '.pastelsdd.yaml'), 'schema: glob-test\n');
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'pastelsdd', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'pastelsdd', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'pastelsdd-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'pastelsdd-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/pastelsdd/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'pastelsdd', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.pastelsdd.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: pastelsdd directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'pastelsdd', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.pastelsdd.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
        expect(metadata).toContain('schema: spec-driven');
      }, 60000);
    });
...
`
    ...
    describe('backwards compatibility', () => {
      ...
      it('changes with metadata work without config file', async () => {
        ...
        await fs.writeFile(
          path.join(changeDir, '.pastelsdd.yaml'),
          'schema: spec-driven\ncreated: "2025-01-05"\n'
        );
