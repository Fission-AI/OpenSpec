...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clearspec-test-config-'));
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse clearspec/config.yaml'),
          expect.anything()
        );
      });
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when clearspec directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit clearspec/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
