...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Pastelsdd CLI');
			expect(script).toContain('$pastelsddCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for pastelsdd command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Pastelsdd',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName pastelsdd');
			expect(script).toContain('-ScriptBlock $pastelsddCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-PastelsddChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-PastelsddSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-PastelsddSchemas');
			expect(script).toContain('pastelsdd __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-PastelsddChanges');
			expect(script).toContain('pastelsdd __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-PastelsddSpecs');
			expect(script).toContain('pastelsdd __complete specs 2>$null');
		});
