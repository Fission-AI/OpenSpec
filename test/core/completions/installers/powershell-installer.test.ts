...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `clearspec-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'ClearSpecCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'ClearSpecCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'ClearSpecCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/ClearSpecCompletion.ps1';

    // Note: CLEARSPEC_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# CLEARSPEC:START - ClearSpec completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# CLEARSPEC:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# CLEARSPEC:END');
      expect(content).not.toContain('# ClearSpec completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for ClearSpec
$clearspecCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName clearspec -ScriptBlock $clearspecCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('ClearSpecCompletion.ps1');
    ...
    // Note: CLEARSPEC_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `clearspec powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('clearspec powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      const textWithBlock = [
        '. "C:\\Code\\profile.ps1"',
        '# CLEARSPEC:START',
        '. "/path/to/ClearSpecCompletion.ps1"',
        '# CLEARSPEC:END',
        '',
      ].join('\n');
      ...
      // Verify content: original line kept, ClearSpec block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Install adds the ClearSpec block
      ...
      // Uninstall removes the ClearSpec block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# CLEARSPEC:START');
      ...
      expect(profileContentAfter).not.toContain('# CLEARSPEC:START');
    });
