...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `clearspec-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `clearspec-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'clearspec-explore');
    await writeSkill(projectDir, 'clearspec-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'clsx', 'my-custom.md');
