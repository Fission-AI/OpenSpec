...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clearspec-test-'));
    });
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, 'clearspec', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .clearspec.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'clearspec', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.clearspec.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clearspec-test-'));

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, 'clearspec');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
