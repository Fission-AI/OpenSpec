...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `clearspec-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing clearspec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No ClearSpec changes directory found. Run 'clearspec init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'clearspec', 'changes');
