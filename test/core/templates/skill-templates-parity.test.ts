...
const EXPECTED_FUNCTION_HASHES: Record<string, string> = {
  ...
  getArchiveChangeSkillTemplate: '6333f3f372ce13a6486baea72e176220a13de57c93c168f3d885b86a670f39c2',
  ...
  getOpsxArchiveCommandTemplate: '68294096d50f850373ceac9ca6880267f08ade52d4fe52b9854dc0baa09c492e',
...
const EXPECTED_GENERATED_SKILL_CONTENT_HASHES: Record<string, string> = {
  ...
  'openspec-archive-change': '50f0cb57bb37059e448c9f93c3f87820e3125c0b0861e3d7fdc40f16a2099d3d',
...
describe('skill templates split parity', () => {
  ...
  it('guides archive sync assessment when the main spec is missing', () => {
    ...
    for (const [name, content] of archiveSurfaces) {
      expect(content, name).toContain('If the corresponding main spec does not exist');
      expect(content, name).toContain('ADDED requirements -> include in sync as creation of a new main spec');
      expect(content, name).toContain('MODIFIED / RENAMED requirements -> warn that they are not allowed without an existing main spec');
      expect(content, name).toContain('REMOVED requirements -> warn that they have no target main spec and do not require sync');
      expect(content, name).toContain('Do not treat a missing main spec as "already synced" or "no sync needed"');
    }
  });
});
