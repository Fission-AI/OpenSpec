...
import { DEFAULT_OPENSPEC_DIR_NAME } from '../../src/core/config.js';
...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    const openspecDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME);
  ...
  describe('execute', () => {
    it('should archive a change successfully', async () => {
      ...
      const changeDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'changes', changeName);
      await fs.mkdir(changeDir, { recursive: true });
      
      // Create a valid delta spec so it passes validation
      ...
      const archiveDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'changes', 'archive');
    ...
    it('should warn about incomplete tasks', async () => {
      ...
      // Execute archive with --yes flag and noValidate
      await archiveCommand.execute(changeName, { yes: true, noValidate: true });
      
      // Verify progress was logged (instead of a warning message prefix)
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Task status: 1/3 tasks')
      );
    });
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      // Execute archive - should succeed with noValidate
      await archiveCommand.execute(changeName, { yes: true, noValidate: true });
      
      // Verify spec was created with only ADDED requirements
      const mainSpecPath = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if change does not exist', async () => {
      await archiveCommand.execute('non-existent-change', { yes: true, noValidate: true });
      expect(console.log).toHaveBeenCalledWith("Change 'non-existent-change' not found.");
      expect(console.log).toHaveBeenCalledWith("Aborted. No files were changed.");
    });
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'changes', 'archive', `${date}-${changeName}`);
    ...
    it('should handle changes without tasks.md', async () => {
      ...
      // Execute archive without tasks.md and noValidate
      await archiveCommand.execute(changeName, { yes: true, noValidate: true });
    ...
    it('should handle changes without specs', async () => {
      ...
      // Execute archive without specs and noValidate
      await archiveCommand.execute(changeName, { yes: true, noValidate: true });
...
- **THEN** it is captured`;
    ...
    it('should proceed with archive when user declines spec updates', async () => {
      ...
      const specDir = path.join(changeDir, 'specs', 'test-capability');
      await fs.mkdir(specDir, { recursive: true });
...
- THEN`;
      ...
      // Mock confirm sequence:
      // 1. "Skipping validation..." -> true (proceed)
      // 2. "Apply spec updates?" -> false (decline)
      mockConfirm
        .mockResolvedValueOnce(true)
        .mockResolvedValueOnce(false);
      
      // Execute archive without --yes flag and noValidate
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
      ...
      // Should log the error and abort
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('gamma REMOVED failed for header "### Requirement: Another Missing" - not found')
      );
      expect(console.log).toHaveBeenCalledWith('Aborted. No files were changed.');
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('delta validation failed - when a rename exists, MODIFIED must reference the NEW header "### Requirement: New"')
      );
      expect(console.log).toHaveBeenCalledWith(
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'epsilon', 'spec.md');
      await fs.mkdir(path.dirname(epsilonMain), { recursive: true });
      await fs.writeFile(epsilonMain, `# epsilon Specification\n\n## Purpose\nEpsilon purpose.\n\n## Requirements\n\n### Requirement: E1\ne1`);

      const zetaMain = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'zeta', 'spec.md');
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when openspec directory does not exist', async () => {
      ...
      await fs.rm(path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME), { recursive: true });
      
      await archiveCommand.execute('any-change', { yes: true, noValidate: true });
      
      expect(console.log).toHaveBeenCalledWith("No OpenSpec changes directory found. Run 'openspec init' first.");
      expect(console.log).toHaveBeenCalledWith("Aborted. No files were changed.");
    });
  });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, DEFAULT_OPENSPEC_DIR_NAME, 'changes', change2), { recursive: true });
      
      // Mock select to return first change
      mockSelect.mockResolvedValueOnce(change1);
      
      // Execute without change name and noValidate
