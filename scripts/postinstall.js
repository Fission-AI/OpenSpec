 ...
 * - ENPALSPEC_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.ENPALSPEC_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'ENPALSPEC_NO_COMPLETIONS=1 set' };
  }
...
async function installCompletions(shell) {
  try {
    ...
    if (!CompletionFactory.isSupported(shell)) {
      console.log(`\nTip: Run 'enpalspec completion install' for shell completions`);
      return;
    }
