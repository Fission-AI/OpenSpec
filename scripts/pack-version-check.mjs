...
function main() {
  ...
  try {
    log(`Packing @enpal/enpalspec@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), 'enpalspec-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', '@enpal', 'enpalspec', 'bin', 'openspec.js');
