...
Commands are always the second case. No skill creates an `openspec/` directory on its own in either case. The entries below describe what each skill does once a root is in place.
