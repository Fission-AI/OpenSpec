...
checkbox format to track progress. Only `- [x]` counts as done; every other
marker, including `- [~]` and an empty `- []`, reads as unfinished. A line
with no checkbox is not tracked at all.
