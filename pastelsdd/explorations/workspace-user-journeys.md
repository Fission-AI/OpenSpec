...
This document describes the exact user experience for Pastelsdd across:
...
- How Pastelsdd decides which specs to read
- How Pastelsdd behaves when roots are missing, ambiguous, or partially available
...
Pastelsdd should operate on **scopes**.
...
- Pastelsdd reads only the relevant specs
- Pastelsdd stores each artifact in the owning place
- If the work spans multiple owning places, Pastelsdd creates a neutral coordination workspace
...
Pastelsdd should model them explicitly as:
...
If there is no owner yet, the shared behavior can live temporarily as an initiative note during exploration, but Pastelsdd should not promote it into a canonical spec until ownership is explicit.
...
Users should still recognize the existing Pastelsdd workflow:
...
The new behavior is that Pastelsdd may ask:
...
Instead, Pastelsdd should create a neutral coordination workspace for the initiative.
...
Pastelsdd may surface them to users and agents, but should not require them to resolve or validate across roots in the first version.

### 6. Shared contracts need explicit ownership

When the user describes behavior that spans multiple boundaries, Pastelsdd should help them decide whether they are creating:

- a local spec
- a shared contract
- an initiative note that is not yet canonical

If the work is a real long-lived contract, Pastelsdd should prompt for a canonical owner rather than burying it in whichever repo the user happened to be standing in.

### 7. Do not force "shared ownership" as implicit co-ownership

Pastelsdd should help teams create **shared contract ownership**, not ambiguous co-ownership.
...
When a user starts in `web-client` and describes a checkout behavior spanning web, iOS, Android, and backend, Pastelsdd should treat the current cwd as a clue about likely consumers, not as proof that `web-client` should own the shared contract.
...
A single Pastelsdd root where canonical specs and changes can live.

In practice a project may be:

- a repo root with `pastelsdd/`
- a monorepo root with one `pastelsdd/`
- a nested owned scope if Pastelsdd later supports nested roots
...
```text
~/work/pastelsdd-workspaces/add-3ds/
  .pastelsdd-workspace/
    workspace.yaml
    initiative.md
    links.yaml
...
```text
monorepo/
  apps/web/pastelsdd.scope.yaml
  services/billing/pastelsdd.scope.yaml
  packages/contracts/pastelsdd.scope.yaml
```
...
Pastelsdd needs to answer two separate questions:
...
Pastelsdd discovers candidate scopes from:
...
- `github.com/thiagodiogo/web-client`
- `github.com/thiagodiogo/contracts`

User shorthand may be accepted:

- `thiagodiogo/web-client`
- `fission/web-client`

Pastelsdd resolves these to local paths using this order:

1. explicit path passed for this run
2. saved local Pastelsdd registry
...
For team use, Pastelsdd should separate:
...
Pastelsdd should communicate this clearly:
...
Pastelsdd should help teams choose the owner that can most credibly do all of the following:
...
When Pastelsdd detects a likely shared contract, it should ask or infer from these questions:
...
Pastelsdd should default to that existing owner.
...
Pastelsdd should suggest that existing domain owner if there is no dedicated shared area.
...
Pastelsdd should help create a new shared contract scope.
...
Pastelsdd should allow the behavior to remain in initiative notes temporarily and explicitly mark it as non-canonical until ownership is chosen.

### What Pastelsdd should suggest by default
...
### What Pastelsdd should not do

Pastelsdd should not:
...
When the user describes a likely cross-boundary behavior, Pastelsdd should treat this as a first-class planning moment.
...
During `/pastel:propose` or `/pastel:explore`, Pastelsdd should ask something like:
...
If the user chooses "create a new shared contract scope," Pastelsdd should guide them.
...
- pastelsdd/specs/contracts/checkout
- pastelsdd/specs/shared/checkout
...
Pastelsdd should avoid auto-creating a brand-new repo. It can scaffold the plan and record the decision, but repo creation may be organizationally sensitive and usually belongs outside the CLI.

### Temporary initiative-only mode

If ownership is unclear, Pastelsdd should support:
...
4. Let Pastelsdd determine whether this is:
   - local only
   - multi-scope in one root
   - cross-root coordination
5. Pastelsdd creates artifacts in the right places
6. Pastelsdd tells the user where to continue planning and where to implement
...
Pastelsdd should support three agent capability levels.
...
- Pastelsdd tells the user exactly which roots to attach
...
- User opens the coordination workspace
- Pastelsdd writes agent-readable absolute paths into the workspace instructions
...
- Pastelsdd guides the user into repo-local implementation sessions afterward
- `/pastel:apply` runs in each repo separately

This must be treated as a first-class case, not a fallback afterthought.

---

## Current Pastelsdd Flow To Preserve

Today the flow is:

1. User enters a repo
2. User runs `pastelsdd init`
3. User opens an agent in that repo
4. User runs `/pastel:explore` or `/pastel:propose`
5. Pastelsdd stores changes in `pastelsdd/changes/<change>/`
6. `/pastel:apply` implements within that repo

That should remain the exact feel for single-repo work.

The only extension is:

- if Pastelsdd detects a cross-root initiative, it explicitly upgrades the workflow into a coordination workspace flow

---

## Journey 1: Single Repo, Standard Pastelsdd Project
...
```text
Created pastelsdd/changes/add-dark-mode/
Using scope: repo
...
Ready for implementation with /pastel:apply
```
...
Pastelsdd archives the change in this repo, as today.
...
- User runs from subdirectory: Pastelsdd should either walk up or clearly tell them to run from repo root
...
```text
/pastel:propose add-invoice-filtering
```

Pastelsdd detects this is still one project root, but there are multiple candidate scopes.

If the user’s request clearly mentions one area, Pastelsdd may infer:
...
1. create a single monorepo-local change under `pastelsdd/changes/add-invoice-filtering/`
...
```text
/pastel:propose add-3ds-checkout
```

Pastelsdd detects likely affected scopes:
...
```text
platform/pastelsdd/changes/add-3ds-checkout/
```
...
Pastelsdd ignores unrelated specs by default.
...
This means Pastelsdd cannot assume:
...
```text
apps/web/pastelsdd.scope.yaml
apps/mobile/pastelsdd.scope.yaml
services/billing/pastelsdd.scope.yaml
packages/contracts/pastelsdd.scope.yaml
```
...
```text
/pastel:propose add-checkout-loading-state
```

Pastelsdd detects this is limited to `apps/web`.
...
At this point Pastelsdd must make a product decision.
...
If the user chooses the coordination flow, Pastelsdd creates:
...
They already use Pastelsdd there.
...
At this point Pastelsdd should not silently create one cross-repo change inside `web-client`.
...
### Expected Pastelsdd behavior

Pastelsdd interrupts the default single-repo flow and says:

```text
This work spans multiple owning roots:
- github.com/thiagodiogo/contracts
- github.com/thiagodiogo/billing-service
- github.com/thiagodiogo/web-client
- github.com/thiagodiogo/ios-client

For cross-repo work, Pastelsdd recommends creating a coordination workspace.
...
If the user agrees, Pastelsdd creates the workspace.
...
```bash
pastelsdd workspace create add-3ds --at ~/work/pastelsdd-workspaces/add-3ds
```
...
Pastelsdd now resolves local paths for:
...
Pastelsdd then tells the user:

```text
Next step:
1. Open your coding agent in ~/work/pastelsdd-workspaces/add-3ds
...
Pastelsdd has generated workspace instructions at:
.pastelsdd-workspace/agents/claude.md
```
...
Or Pastelsdd may have already scaffolded the initiative and tell the agent to continue it.
...
- `contracts/pastelsdd/changes/add-3ds-contract/`
- `billing-service/pastelsdd/changes/add-3ds-billing/`
- `web-client/pastelsdd/changes/add-3ds-web/`
- `ios-client/pastelsdd/changes/add-3ds-ios/`
...
```bash
pastelsdd initiative new add-3ds
```
...
Pastelsdd resolves local clones and writes the workspace files.

### Agent setup

The user opens the coordination workspace in their agent.

Pastelsdd-generated agent instructions contain:
...
Pastelsdd should support that directly.
...
```text
~/work/pastelsdd-initiatives/
```

Or a team-owned repo such as:

```text
github.com/thiagodiogo/initiatives
```

Inside it, Pastelsdd creates:
...
Each teammate keeps local path mappings outside the shared repo, for example in Pastelsdd local config/data:

- `github.com/thiagodiogo/contracts` -> `/Users/alice/src/contracts`
- `github.com/thiagodiogo/contracts` -> `/home/bob/work/contracts`
...
2. runs something like `pastelsdd workspace doctor` or `pastelsdd workspace sync`
...
Pastelsdd generates agent instructions using:
...
Pastelsdd should support the initiative being created in a shared coordination repo owned by the sponsoring or driving team.
...
When another team member opens the shared coordination workspace, Pastelsdd should make this explicit:
...
Pastelsdd should still allow planning.
...
```yaml
projects:
  - id: github.com/thiagodiogo/contracts
    path: /Users/me/work/contracts
    status: resolved
  - id: github.com/thiagodiogo/billing-service
    status: unresolved
  - id: github.com/thiagodiogo/web-client
    path: /Users/me/work/web-client
    status: resolved
  - id: github.com/thiagodiogo/ios-client
    status: unresolved
```
...
Pastelsdd should never blindly read all specs from all roots.
...
```text
Related references:
- github.com/thiagodiogo/contracts: pastelsdd/specs/checkout/spec.md
- github.com/thiagodiogo/web-client: pastelsdd/specs/checkout/web/spec.md
```
...
When a selected change involves a shared contract, Pastelsdd should prefer this read order:
...
If a cross-boundary behavior has not yet been assigned a canonical owner, Pastelsdd should store it as initiative-level draft material rather than pretending it is already a canonical spec.
...
## Journey 10: `/pastel:apply` In Cross-Root Work
...
Next suggested step:
- open /Users/me/work/contracts and run /pastel:apply add-3ds-contract
```
...
Should Pastelsdd treat this as:
- A shared contract
...
Pastelsdd detects this likely requires a shared contract.
...
If the user selects an existing contracts repo, Pastelsdd:
...
If the user keeps it initiative-only, Pastelsdd:
...
Pastelsdd checks linked repo changes:
...
4. run `pastelsdd update`
5. open agent in monorepo root

During planning, Pastelsdd asks for scope selection when needed.
...
1. each repo independently runs `pastelsdd init`
...
When Pastelsdd creates a coordination workspace, it should generate agent-facing files.
...
- Pastelsdd recommends creating a coordination workspace
...
- `pastelsdd workspace doctor` or equivalent relinks roots
...
If Pastelsdd adopts these journeys, several design conclusions follow.

### 1. Multi-repo planning needs a first-class coordination workspace

Without this, Pastelsdd has no honest answer to where the user should stand or where the cross-repo initiative should live.

### 2. Repo-local changes and canonical specs should stay in owners

Without this, Pastelsdd creates duplicate or misleading sources of truth.

### 3. Large monorepos cannot be treated as always-simple single roots

Pastelsdd must support both:
...
Pastelsdd must separately handle:
...
Cross-root planning only works reliably if Pastelsdd writes deterministic workspace context for the agent.

### 6. References should stay informational in v1

If Pastelsdd validates cross-root references by default, it has effectively shipped a dependency graph system.
...
Pastelsdd should help users choose among:
...
To support teams and orgs cleanly, Pastelsdd should distinguish:
...
1. Should a coordination workspace always be user-visible on disk, or may it optionally live in global Pastelsdd data directories?
...
3. Should Pastelsdd support nested `pastelsdd/` roots, or keep one root and model scopes separately?
