...
> Pastelsdd should have one default path, one escalation path, and fewer explicit concepts shown to the user unless the system actually needs a decision from them.
...
2. Cross-root work feels like a workflow restart:
   ...
   - Pastelsdd says this is multi-repo
...
- "Pastelsdd figures out whether this stays local or needs to expand"
- "If it expands, it carries me forward instead of making me restart"
- "I only see advanced concepts when Pastelsdd needs a real decision from me"
...
2. Run `/pastel:explore` or `/pastel:propose`
3. Pastelsdd plans locally unless it has a strong reason not to
...
> This work spans multiple owned areas strongly enough that Pastelsdd needs to upgrade it into a coordinated initiative.
...
- "Pastelsdd is upgrading this into a coordinated initiative"
...
### Concepts Pastelsdd should keep implicit until needed
...
### Concepts Pastelsdd should only show when a real decision is needed
...
If Pastelsdd realizes the work is no longer local, it should escalate in one motion.
...
If escalation is needed, Pastelsdd should do as much as possible automatically:
...
Pastelsdd can upgrade this into a coordinated initiative.

Suggested next step:
- create a coordination workspace at ~/work/pastelsdd-workspaces/add-3ds
...
When Pastelsdd has to ask questions, it should ask the smallest useful set.
...
Only ask if Pastelsdd has strong evidence of a cross-boundary contract.
...
To keep the UX intuitive, Pastelsdd should aggressively choose defaults.
...
Pastelsdd should infer affected areas from:
...
If an existing shared contract owner already exists, Pastelsdd should suggest it instead of asking an abstract ownership question.
...
Pastelsdd can start with a local coordination workspace and later offer:
...
2. Pastelsdd upgrades to a coordinated initiative if needed
3. When the work becomes collaborative, Pastelsdd offers to make it team-shared
...
Run `pastelsdd workspace doctor`.
...
Pastelsdd should avoid asking users to classify work in abstract ways if it can infer a reasonable default.
...
By default, Pastelsdd should feel like:
...
- progressive disclosure: only show advanced concepts when Pastelsdd needs a real decision

If Pastelsdd does this well, the same system can feel intuitive for:
