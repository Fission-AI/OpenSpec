...
```
pastelsdd profile set core|extended
pastelsdd profile install <workflow>
pastelsdd profile uninstall <workflow>
pastelsdd profile list
pastelsdd profile show
pastelsdd config set delivery skills|commands|both
pastelsdd config get delivery
...
**After (simplified):**
```
pastelsdd config profile          # interactive picker (delivery + workflows)
pastelsdd config profile core     # preset shortcut
pastelsdd config profile extended # preset shortcut
```
...
- **Global** = user's defaults (`~/.config/pastelsdd/`)
- **Project** = repo-level config (`.pastelsdd/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `pastelsdd init`
2. `pastelsdd init` applies current profile to project
...
### Current Pastelsdd Architecture

Pastelsdd currently assumes:
- One `pastelsdd/` per repo, always at root
...
## Part 4: Three Models for Pastelsdd
...
├── services/
│   └── billing/
│       └── pastelsdd/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── pastelsdd/specs/checkout/spec.md
│   ├── ios/
│   │   └── pastelsdd/specs/checkout/spec.md
│   └── android/
│       └── pastelsdd/specs/checkout/spec.md
└── pastelsdd/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `pastelsdd/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `pastelsdd/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `pastelsdd/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `pastelsdd/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
~/work/
├── billing-service/
│   └── pastelsdd/specs/billing/
├── web-client/
│   └── pastelsdd/specs/checkout/
├── ios-client/
│   └── pastelsdd/specs/checkout/
...
2. **Where do cross-repo changes live?**
   ...
   - In `~/.config/pastelsdd/workspaces/my-platform/changes/`?
...
> **A workspace is a collection of Pastelsdd roots that can be operated on together.**

```yaml
# ~/.config/pastelsdd/workspaces.yaml (or similar)
workspaces:
  my-platform:
    roots:
...
- `pastelsdd/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `pastelsdd/specs/_shared/checkout-contract/spec.md`
- Or `pastelsdd/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single Pastelsdd root (one `pastelsdd/` directory) |
...
### 4. Does Pastelsdd need to understand dependencies?

If `checkout-web` depends on `checkout-contract`:
- Should Pastelsdd know this relationship?
...
**Ambitious:**
> Pastelsdd automatically understands your repo structure, detects cross-cutting specs, and helps you create changes that flow to the right places.

**Simpler:**
> You organize specs however you want. Pastelsdd just works.
...
### What is a spec in Pastelsdd?

For Pastelsdd, a spec should be treated as a **verifiable behavior contract at a boundary**:
...
Pastelsdd is often agent-authored from human exploration. To make that reliable:
...
2. `pastelsdd/specs/pastelsdd-conventions/spec.md` for normative spec conventions
3. `pastelsdd/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules

---

## Part 10: Design Decisions (April 2026)

After evaluating the models above against real multi-repo use cases (see [#725](https://github.com/thiagodiogo/Pastelsdd/issues/725)), we converged on the following design direction.
...
- **Each repo keeps its own canonical `pastelsdd/`** — no change to the fundamental storage model.
...
For larger multi-team work, repo-centric planning is the wrong primary abstraction. Teams and repos are many-to-many facets over the same work. Pastelsdd should treat the **initiative / plan** as the first-class planning object, then link repo-local changes to it.
...
```yaml
# billing-service/pastelsdd/changes/add-3ds/.pastelsdd.yaml
schema: spec-driven
created: 2026-04-12
initiative: add-3ds
links:
...
2. **Local Pastelsdd repo registry** — a persistent mapping in `~/.config/pastelsdd/` or `~/.local/share/pastelsdd/` (see `src/core/global-config.ts`).
...
```yaml
# web-client/pastelsdd/specs/checkout/spec.md frontmatter
references:
...
This avoids accidentally committing Pastelsdd to a full dependency graph system before the use cases justify it.
...
|---------|----------|------------|
| **Spec organization** | Nested specs inside one `pastelsdd/` (Model B) | Each repo has its own `pastelsdd/` |
...
|----------|--------|-------|
| Profile UX | Decided | `pastelsdd config profile` with presets |
