...
Pastelsdd currently has no visibility into how the tool is being used. Without analytics, we cannot:
...
- Store anonymous ID in global config (`~/.config/pastelsdd/config.json`)
- Respect `DO_NOT_TRACK` and `PASTELSDD_TELEMETRY=0` environment variables
