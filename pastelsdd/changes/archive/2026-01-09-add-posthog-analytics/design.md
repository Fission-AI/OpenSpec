...
Pastelsdd needs usage analytics to understand adoption and inform product decisions. PostHog provides a privacy-conscious analytics platform suitable for open source projects.
...
```bash
PASTELSDD_TELEMETRY=0    # Disable telemetry
...
- Full `pastelsdd telemetry` command - Can add later if users request
...
```typescript
{
  ...
  properties: {
    ...
    version: '1.2.3'      // Pastelsdd version
  }
}
```
