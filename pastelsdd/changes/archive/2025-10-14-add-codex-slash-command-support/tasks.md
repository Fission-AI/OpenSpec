...
- [x] 1.2 Implement a `CodexSlashCommandConfigurator` that writes `.codex/prompts/pastelsdd-{proposal,apply,archive}.md`, ensuring the prompt directory exists and wrapping content in Pastelsdd markers.
...
- [x] 3.1 Update the `pastelsdd update` flow to refresh existing Codex prompts without creating new ones when files are missing.
...
- [x] 4.2 Add a release note snippet that points Codex users to the generated `/pastelsdd-proposal`, `/pastelsdd-apply`, and `/pastelsdd-archive` commands.
