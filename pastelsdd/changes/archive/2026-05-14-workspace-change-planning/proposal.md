...
- install and refresh Pastelsdd agent skills from the workspace root so agents can operate from the planning home
...
Workspace setup and update may write agent skill files into the workspace root, such as `.codex/skills/` or `.claude/skills/`, because those files make the workspace planning home usable by agents. That setup work must not write Pastelsdd artifacts or agent skill files into linked repos or folders.

Interactive setup should ask which agents should get Pastelsdd skills in the workspace, preselecting the preferred opener when that opener supports skills. Workspace update should let users refresh or change those installed agent skills later, including when run from inside the workspace.
...
`pastelsdd config profile` should remain global, but when it runs from inside an Pastelsdd workspace and changes the global profile or delivery settings, it should offer to apply the new workflow selection to the current workspace by running `pastelsdd workspace update`.

Workspace-local skill selection should be machine-local state: setup records which agents received skills, update refreshes that stored selection by default, and explicit `--tools` changes the stored selection. Pastelsdd should detect when workspace-local skills drift from the current global profile and give clear update guidance.
...
Workspace help, docs, and completions should make the distinction legible: `pastelsdd update` remains repo/project sync, while `pastelsdd workspace update` syncs workspace-local agent skills.
...
- `cli-config`: Makes `pastelsdd config profile` aware of workspace roots and able to apply global profile changes to the current workspace.
...
- `pastelsdd-conventions`: Defines the relationship between workspace-level planning and repo-local implementation work.
...
- Workspace-aware `pastelsdd config profile` apply prompt behavior.
