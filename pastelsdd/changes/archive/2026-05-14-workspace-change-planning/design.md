...
Workspace setup already creates a planning home, records linked repos or folders, stores a preferred opener, and maintains the root open surface. For workspace change planning to work in practice, the opened agent also needs Pastelsdd workflow skills available from that workspace root.

Repo-local `pastelsdd init` and `pastelsdd update` already provide the user model for choosing agent surfaces and generating skills. Workspace setup should feel similar, but the installation target is the workspace root rather than any linked repo or folder.

The existing artifact workflow assumes a change lives under a repo-local `pastelsdd/changes/<id>` path. Workspace planning needs the same workflow vocabulary, but the planning home may be a workspace root and the implementation homes may be linked repos or folders.

## Goals / Non-Goals

**Goals:**
- Install Pastelsdd agent skills into the workspace root during workspace setup.
...
- Let `pastelsdd config profile` offer to apply changed profile settings to the current workspace when run from inside a workspace.
- Redirect workspace users from repo-local `pastelsdd update` to `pastelsdd workspace update`.
...
- Changing repo-local `pastelsdd init` or `pastelsdd update` behavior.

## Decisions

### Use agent-skill language in workspace UX

Workspace setup should ask, "Which agents should get Pastelsdd skills in this workspace?" rather than using the broader "AI tools" wording. The user-visible action is installing skills for coding agents, and the target is the workspace planning home.
...
Alternative considered: install skills only for the preferred opener. That is simpler, but opener choice means "how should I open this workspace" while skill selection means "which agents should understand Pastelsdd here."

### Persist selected workspace skill agents locally

Workspace setup should store the selected skill-capable agents in `.pastelsdd-workspace/local.yaml` because agent paths and installed tool surfaces are machine-local. Workspace update should use that stored selection when the user does not pass `--tools` or make a new interactive selection.

Explicit `--tools` on workspace setup/update should replace the stored selection. `--tools none` should store an empty selection and remove only known Pastelsdd-managed workspace skill directories.
...
Alternative considered: infer selected agents by scanning `.codex/skills/`, `.claude/skills/`, and similar directories. Scanning is useful as a fallback, but persisted selection gives predictable update behavior and avoids treating unrelated user-authored files as Pastelsdd-managed state.

### Keep non-interactive setup backward-compatible

`pastelsdd workspace setup --no-interactive` should not require `--tools`. If `--tools` is omitted, setup should create the workspace and skip skill installation, preserving existing scripted workspace setup behavior. Human and JSON output should say that no workspace skills were installed and that `pastelsdd workspace update --tools <ids>` can add them later.

`pastelsdd workspace update --no-interactive` without `--tools` should refresh the stored workspace skill agent selection. If no selection is stored, it should complete without installing skills and report a clear no-op with guidance to pass `--tools`.
...
`pastelsdd workspace update` should refresh, add, or remove workspace-local Pastelsdd skills after setup. It should resolve the current workspace when run from inside a workspace, and also support named and non-interactive forms.

Workspace update should compare the active global profile's workflow selection with the last applied workspace skill state. If they differ, update should add/remove only Pastelsdd-managed workflow skill directories for the selected agents. Workspace doctor/list/status surfaces may report the drift as a warning, and `pastelsdd config profile` no-op inside a workspace should use the same drift check for guidance.

Alternative considered: reuse `pastelsdd update` from inside the workspace. That command currently means repo/project update, while workspace update needs workspace selection, workspace JSON/status behavior, and linked-repo safety rules.

### Make `config profile` workspace-aware

`pastelsdd config profile` should remain a global configuration command. When it runs inside a repo-local Pastelsdd project and the user chooses to apply changes, it should continue to run `pastelsdd update`.

When it runs inside an Pastelsdd workspace and the profile or delivery settings actually change, it should prompt to apply changes to the current workspace. If confirmed, it should run `pastelsdd workspace update` for that workspace. If declined, it should explain that the global config changed and the user can run `pastelsdd workspace update` later.

The preset shortcut `pastelsdd config profile core` should keep its non-interactive character and not launch an apply prompt. When run from inside a workspace, it should save global config and print workspace-specific follow-up guidance to run `pastelsdd workspace update`. When run inside a repo-local project, it should keep the existing repo-local guidance.
...
If a directory is both inside a workspace planning home and inside a repo-local Pastelsdd project, the nearest planning home should determine the apply prompt. This avoids applying a workspace profile change to a linked repo when the user is intentionally operating from the workspace planning home.

Alternative considered: make `pastelsdd config profile` update all known workspaces. That would be convenient in small setups, but global config changes should not fan out into multiple planning homes without an explicit per-workspace action.
...
Workspace changes should live under the workspace planning path, initially `changes/<id>` at the workspace root. Creating the workspace change should capture shared intent once and may record affected areas, but it should not create repo-local `pastelsdd/changes/<id>` directories in linked repos.
...
```text
changes/<id>/
  .pastelsdd.yaml          # schema: workspace-planning
...
The first schema should stay intentionally close to the normal Pastelsdd artifact shape: proposal, specs, design, and tasks. Area-specific requirements live under `specs/` and area-specific work can be represented as sections in `tasks.md`. This slice does not introduce another area manifest beside those normal planning artifacts.
...
`pastelsdd status --change <id> --json` should become the primary source of machine-readable action context. It should include the planning home, change root, concrete artifact paths, affected areas, next steps, and constraints such as allowed edit roots when implementation is later in scope.
...
Generated workflow skills should ask Pastelsdd where artifacts live instead of embedding repo-local paths such as `pastelsdd/changes/<name>`. The standard skill pattern should be:

```text
1. Run `pastelsdd status --change "<name>" --json`.
2. Use the returned planning home, artifacts, next steps, and action context.
3. Run `pastelsdd instructions <artifact> --change "<name>" --json` before writing an artifact.
...
Alternative considered: add a new `pastelsdd context` command now. That may become useful, but it adds a new surface before we have proven that enriched status/instructions are insufficient.
...
`pastelsdd update` should remain the repo/project update command. When it is run from an Pastelsdd workspace planning home, it should not try to treat the workspace as a repo-local project. It should fail or redirect with clear guidance to run `pastelsdd workspace update`.

Alternative considered: make `pastelsdd update` polymorphic and perform workspace update inside workspaces. That would be convenient, but it blurs the repo/project versus workspace boundary this change is trying to make explicit.

### Update docs, help, and completions

The CLI help, command registry/completions, and user docs should include `pastelsdd workspace update`, its `--tools` behavior, the global-profile relationship, and the skills-only workspace delivery rule.
...
- Scoped plan files may eventually attach at the change, phase, affected-area, or work-focus level. This slice intentionally keeps the first workspace schema close to normal Pastelsdd artifacts: proposal, specs, design, and tasks.
...
- Workflow skills may later separate generic Pastelsdd workflow semantics from agent-specific affordances such as asking questions, tracking todos, or delegating work. This slice only makes generated workflow skills path-agnostic.
- Pastelsdd may need a named exploratory-notes convention for preserving unsettled thinking before it is promoted into proposal, design, specs, or tasks. This cleanup keeps the current change folder focused on standard artifacts.
...
- Removing unselected skills could remove user-modified files → remove only known Pastelsdd-managed workflow skill directories by explicit workflow list.
