...
I create an Pastelsdd workspace.
That workspace is where changes live.
My code stays where it is.
Pastelsdd links the workspace to those local paths.
```
...
Users should not have to choose a storage location, create a change early, or understand internal workspace state before Pastelsdd can orient itself.
...
This change defines the user-facing foundation for Pastelsdd workspaces.

An Pastelsdd workspace has a recognizable planning home:
...
`changes/` is where workspace-level planning lives. `.pastelsdd-workspace/` identifies the directory as an Pastelsdd workspace and stores workspace state.

Pastelsdd-managed workspaces live in one standard location:

```text
<global-data-dir>/workspaces/
```

Users should not need to choose that location. Pastelsdd still shows the workspace path after setup so users know where planning files live. This foundation slice does not provide a workspace-specific environment-variable or configuration override for managed workspace storage.

Pastelsdd also keeps a lightweight local registry of known workspaces on the current machine. The registry powers global commands, pickers, and listing, but each workspace folder remains the source of truth.
...
A linked path can be a full repo, a folder inside a monorepo, or another existing folder the workspace should plan against. A linked path does not need repo-local `pastelsdd/` state before it can be included in workspace planning. Repo-local Pastelsdd state may still matter later for implementation, verification, or archive workflows, but it is not a prerequisite for planning visibility.

Native Windows/PowerShell and WSL2 are both supported. Each runtime uses its own path conventions. Pastelsdd does not translate paths between Windows and WSL in this foundation slice.

## Outcome

After this change, later workspace features can rely on one clear product contract:

- Pastelsdd can tell when the user is inside a workspace.
- Pastelsdd knows where to create managed workspaces by default.
- Pastelsdd can keep a local registry of known workspaces.
- A workspace has one visible planning area: `changes/`.
- Workspace state is distinguishable from repo-local `pastelsdd/` state.
...
- Linked repos or folders do not need repo-local Pastelsdd state for workspace planning.
...
- The POC's hidden `.pastelsdd/` workspace metadata shape made workspace state too easy to confuse with repo-local Pastelsdd state.
- Users should not need to run repo-local `pastelsdd init` inside the workspace root.
- The POC's requirement that registered repos already have `pastelsdd/` is too strict for planning. Repos and folders should be linkable before they adopt repo-local Pastelsdd state.
...
- Workspace identity directory: `.pastelsdd-workspace/`.
- Workspace identity file: `.pastelsdd-workspace/workspace.yaml`.
...
- Local machine state: `.pastelsdd-workspace/local.yaml`.
- Local machine state exclusion: Pastelsdd-created workspaces exclude `.pastelsdd-workspace/local.yaml` from portable collaboration state by default.
- Local workspace registry: `<global-data-dir>/workspaces/registry.yaml`.
- Default workspace base: `<global-data-dir>/workspaces/`.
- Platform behavior: native Windows and WSL2 each use the path conventions of the runtime running Pastelsdd.
...
- Repo-local `pastelsdd/` state is not required for workspace planning visibility.
...
- No complete `pastelsdd workspace setup`, `pastelsdd workspace link`, or `pastelsdd workspace relink` flow yet.
- No public `pastelsdd workspace create` command in the first user-facing workspace flow.
- No user-facing command, environment variable, or configuration setting for changing the standard workspace location.
- No question that asks users where Pastelsdd should store workspaces by default.
...
- `workspace-foundation`: Defines the product foundation for Pastelsdd workspaces.

### Modified Capabilities

- `pastelsdd-conventions`: Describes how coordination workspaces differ from repo-local Pastelsdd projects.
