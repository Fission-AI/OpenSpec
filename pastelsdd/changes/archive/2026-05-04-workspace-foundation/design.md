...
An Pastelsdd workspace is the durable planning home for work that spans multiple repos or folders.
...
The foundation intentionally avoids the rest of the workflow. It only defines how Pastelsdd recognizes a workspace, where managed workspaces live, how linked paths are represented, and how shared state differs from local state.
...
Pastelsdd workspaces use this shape:
...
The user-facing planning surface is `changes/`. The identity file that makes the directory a workspace is `.pastelsdd-workspace/workspace.yaml`.
...
Users should not run repo-local `pastelsdd init` inside the workspace root. A workspace is already an Pastelsdd coordination surface; it is not a product repo adopting repo-local Pastelsdd.
...
Pastelsdd should not maintain a cross-platform reserved-name list in this slice. Setup/create flows should let filesystem creation surface OS-specific invalid folder names, then report that failure clearly.

The same workspace name is stored in `.pastelsdd-workspace/workspace.yaml`, used as the default managed workspace folder name, and used as the local registry name.
...
Pastelsdd-created workspaces should include an ignore rule for `.pastelsdd-workspace/local.yaml` so local checkout paths are not accidentally shared. `.pastelsdd-workspace/workspace.yaml` remains the portable workspace identity and link-name state.

## Workspace Location

Pastelsdd should create managed workspaces in one standard place:

```text
getGlobalDataDir()/workspaces
```

That reuses existing Pastelsdd data-directory behavior:

- `$XDG_DATA_HOME/pastelsdd/workspaces` when `XDG_DATA_HOME` is set
- `~/.local/share/pastelsdd/workspaces` on Unix/macOS fallback
- `%LOCALAPPDATA%\pastelsdd\workspaces` on native Windows fallback
...
Pastelsdd should show the resolved workspace path after setup. Quiet defaults should avoid a prompt, not hide where planning files were created.

## Local Workspace Registry

Pastelsdd should keep a lightweight local registry of known workspaces:
...
version: 1
workspaces:
  platform: /Users/tabish/.local/share/pastelsdd/workspaces/platform
  checkout: /Users/tabish/.local/share/pastelsdd/workspaces/checkout
```
...
Each workspace folder remains authoritative for its own `.pastelsdd-workspace/workspace.yaml` and `.pastelsdd-workspace/local.yaml`. If a registry entry points at a missing or invalid workspace, later check/list flows can report that and suggest a repair.
...
```text
PowerShell:
  default base -> %LOCALAPPDATA%\pastelsdd\workspaces

WSL2:
  default base -> ~/.local/share/pastelsdd/workspaces
```
...
Workspace planning visibility should not require repo-local Pastelsdd state.

That matters for two common cases:

- a repo has not adopted Pastelsdd yet, but still needs to be considered in planning
- a large monorepo has folders such as packages, services, or apps that should be planned like separate areas, without each folder having its own `pastelsdd/`
...
Repo-local spec availability is computed when needed. For example, `repo_specs_path` can be reported by a later doctor command when a linked path contains `pastelsdd/specs`, but that path should not be treated as required workspace state.
