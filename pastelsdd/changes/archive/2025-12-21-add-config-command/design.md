...
The `global-config` spec defines how Pastelsdd reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
**Examples:**
```bash
pastelsdd config get featureFlags              # Returns object
pastelsdd config get featureFlags.experimental # Returns nested value
pastelsdd config set featureFlags.newFlag true
```
...
**Rationale:**
- Raw output enables piping: `VAR=$(pastelsdd config get key)`
