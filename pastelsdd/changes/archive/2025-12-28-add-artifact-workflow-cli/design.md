...
```
pastelsdd status --change <id>
pastelsdd next --change <id>
pastelsdd instructions <artifact> --change <id>
pastelsdd templates [--schema <name>]
...
```bash
pastelsdd status --change add-auth   # explicit, works
pastelsdd status                      # error: missing --change
```
...
```bash
pastelsdd new change add-feature
```

**Rationale:**
- `pastelsdd new <name>` is ambiguous (new what?)
- `pastelsdd new change <name>` is clear and extensible
- Can add `pastelsdd new spec <name>` later if needed
