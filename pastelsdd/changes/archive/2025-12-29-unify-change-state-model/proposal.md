...
Result: `pastelsdd new change foo && pastelsdd view` shows `foo` as "Completed" when it has no content.
...
Result: `pastelsdd status --change foo` says "not found" even though the directory exists.
...
|---------|--------|-------|
| `pastelsdd view` | Empty = "Completed" | Empty = "Draft" |
| `pastelsdd status --change X` | Requires proposal.md | Works on any directory |
| `pastelsdd validate X` | Requires proposal.md | Unchanged (still requires it) |

## Breaking Changes

### Minimal Breaking Change

1. **`pastelsdd view` output**: Empty changes move from "Completed" section to new "Draft" section

### Non-Breaking

- Active/Completed semantics unchanged (still task-based)
- `getActiveChangeIds()` unchanged
- `pastelsdd validate` unchanged
...
|---------|-----------------------------|-----------------------------|----------------|
| `pastelsdd view` | No (reads dirs directly) | Yes → Draft section | **Yes** |
| `pastelsdd list` | No (reads dirs directly) | Yes (shows "No tasks") | No |
| `pastelsdd status/next/instructions` | Yes | Yes | **Yes** |
| `pastelsdd validate` | Yes | No (can't validate empty) | No |
| `pastelsdd show` | Yes | No (nothing to show) | No |
| Tab completions | Yes | Future enhancement | No |

## Success Criteria

1. `pastelsdd new change foo && pastelsdd view` shows `foo` in "Draft" section
2. `pastelsdd new change foo && pastelsdd status --change foo` works
