...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `pastelsdd view`, `pastelsdd list` |
| **Artifact Progress** | Track planning/spec work | `pastelsdd status`, `pastelsdd next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  const changePath = path.join(projectRoot, 'pastelsdd', 'changes', changeName);
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'pastelsdd', 'changes'),
      { withFileTypes: true }
    );
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: pastelsdd new change <name>');
    }
...
```bash
# Before
$ pastelsdd new change foo
$ pastelsdd status --change foo
