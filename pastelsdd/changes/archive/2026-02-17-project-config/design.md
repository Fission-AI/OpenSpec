...
Pastelsdd currently has a fixed schema resolution order:
1. `--schema` CLI flag
2. `.pastelsdd.yaml` in change directory
...
The proposal introduces `pastelsdd/config.yaml` as a lightweight customization layer that sits between preset schemas and full forking. It allows teams to:
...
**Key stakeholders:**
- Pastelsdd users who need light customization without forking
- Teams sharing workflow conventions via committed config

## Goals / Non-Goals

**Goals:**
- Load and parse `pastelsdd/config.yaml` using Zod schema
...
- Config management commands (`pastelsdd config init`) - manual creation for now
...
- Consistent with `.pastelsdd.yaml` used in changes
...
### 2. Config Location: Project Root vs pastelsdd/ Directory

**Decision:** `./pastelsdd/config.yaml` (inside pastelsdd directory)

**Rationale:**
- Co-located with `pastelsdd/schemas/` (project-local-schemas)
- Keeps project root clean
- Natural namespace for Pastelsdd configuration
- Mirrors structure used by other tools (e.g., `.github/`)

**Alternatives considered:**
- `./pastelsdd.config.yaml` in root: Pollutes root, less clear ownership
...
3. **`pastelsdd/config.yaml` schema field** (NEW - project default)
...
**Rationale:**
- Syntax errors in config shouldn't break all of Pastelsdd
- User can fix config incrementally
- Commands remain usable during config development

**Warning message:**
```
⚠️  Failed to parse pastelsdd/config.yaml: [error details]
 ...
 * - Helps understand Pastelsdd's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other Pastelsdd schemas
 */
...
/**
 * Read and parse pastelsdd/config.yaml from project root.
...
export function readProjectConfig(): ProjectConfig | null {
  ...
  let configPath = path.join(projectRoot, 'pastelsdd', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'pastelsdd', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`⚠️  pastelsdd/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`⚠️  Failed to parse pastelsdd/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `❌ Schema '${invalidSchemaName}' not found in pastelsdd/config.yaml\n\n`;
  ...
  message += `\nFix: Edit pastelsdd/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
...
```typescript
// Example: "pastelsdd instructions proposal --change my-feature"
...
export function readProjectConfig(): ProjectConfig | null {
  const projectRoot = findProjectRoot();
  const configPath = path.join(projectRoot, 'pastelsdd', 'config.yaml');
...
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  User runs: pastelsdd instructions proposal --change foo     │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│                                                              │
│  1. Check CLI flag ✗                                         │
│  2. Check .pastelsdd.yaml ✗                                   │
│  3. Check pastelsdd/config.yaml ✓ → "spec-driven"             │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
**[Risk]** YAML parsing errors break Pastelsdd commands
...
→ **Mitigation:** Document common artifact IDs, show in `pastelsdd status` output
