...
Add `pastelsdd/config.yaml` support for project-level configuration. This enables teams to customize Pastelsdd behavior without forking schemas, by providing context and rules that are injected into artifact generation.

## Motivation

Currently, customizing Pastelsdd requires forking entire schemas:
- Must copy all files even to add one rule
- Lose updates when pastelsdd upgrades
...
Pastelsdd customization follows two distinct paths:
...
│   + add rules                                                   │
│                                                                 │
│   pastelsdd/config.yaml            pastelsdd/schemas/my-flow/     │
│                                                                 │
│   ✓ Simple                        ✓ Full control                │
...
- Commands run on changes without `.pastelsdd.yaml` metadata
...
3. **Avoids premature API surface** - No standalone `pastelsdd config init` until feature graduates
...
```
Today (Experimental):
  pastelsdd artifact-experimental-setup
    → prompts for config creation
    → creates .claude/skills/
    → creates pastelsdd/config.yaml

Future (When graduating):
  pastelsdd init
    → prompts for config creation
    → creates pastelsdd/ directory
    → creates pastelsdd/config.yaml

  + standalone commands:
    pastelsdd config init
    pastelsdd config validate
    pastelsdd config set <key> <value>
```

**Why optional?**

Config is **additive**, not required:
- Pastelsdd works without config (uses defaults)
...
- Update `pastelsdd new change` to use config's schema as default
...
- Integration with standard `pastelsdd init` (will add when experimental graduates)
- Standalone `pastelsdd config init` command (may add in future change)
- `pastelsdd config validate` command (may add in future change)
...
✓ Created .claude/skills/pastelsdd-explore/SKILL.md
✓ Created .claude/skills/pastelsdd-new-change/SKILL.md
✓ Created .claude/skills/pastelsdd-continue-change/SKILL.md
✓ Created .claude/skills/pastelsdd-apply-change/SKILL.md
✓ Created .claude/skills/pastelsdd-ff-change/SKILL.md
✓ Created .claude/skills/pastelsdd-sync-specs/SKILL.md
✓ Created .claude/skills/pastelsdd-archive-change/SKILL.md

✓ Created .claude/commands/pastel/explore.md
✓ Created .claude/commands/pastel/new.md
✓ Created .claude/commands/pastel/continue.md
✓ Created .claude/commands/pastel/apply.md
✓ Created .claude/commands/pastel/ff.md
✓ Created .claude/commands/pastel/sync.md
✓ Created .claude/commands/pastel/archive.md

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 Project Configuration (Optional)

Configure project defaults for Pastelsdd workflows.

? Create pastelsdd/config.yaml? (Y/n) Y
...
✓ Created pastelsdd/config.yaml

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎉 Setup Complete!

📖 Config created at: pastelsdd/config.yaml
...
To share with team:
  git add pastelsdd/config.yaml .claude/
  git commit -m "Setup Pastelsdd experimental workflow with project config"
...
pastelsdd instructions proposal --change my-feature
...
pastelsdd instructions specs --change my-feature
...
```bash
pastelsdd instructions design --change my-feature
...
Always at `./pastelsdd/config.yaml` relative to project root. No XDG/global config for simplicity.
...
2. .pastelsdd.yaml in change directory   # Change-specific binding
3. pastelsdd/config.yaml schema field    # Project default (NEW)
...
6. **Create and validate config:**
   ...
   - Write to `pastelsdd/config.yaml` using YAML serializer
...
When `pastelsdd/config.yaml` already exists:
...
ℹ️  pastelsdd/config.yaml already exists. Skipping config creation.

   To update config, edit pastelsdd/config.yaml manually or:
   1. Delete pastelsdd/config.yaml
   2. Run pastelsdd artifact-experimental-setup again
...
try {
  ...
  if (configResult.createConfig) {
    ...
    console.log('✓ Created pastelsdd/config.yaml');
  }
} catch (error) {
...
1. **@inquirer/prompts** (recommended)
   ...
   - Already used in Pastelsdd (if not, lightweight addition)
...
- Verify change's `.pastelsdd.yaml` overrides config
...
- Test all four levels of schema resolution:
  ...
  2. Change metadata `.pastelsdd.yaml`
...
- Existing changes with `.pastelsdd.yaml` metadata aren't affected by config
