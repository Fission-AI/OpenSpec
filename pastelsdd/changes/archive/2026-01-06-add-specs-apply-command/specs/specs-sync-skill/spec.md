...
The system SHALL provide an `/pastel:sync` skill that syncs delta specs from a change to the main specs.

#### Scenario: Sync delta specs to main specs
- **WHEN** agent executes `/pastel:sync` with a change name
- **THEN** the agent reads delta specs from `pastelsdd/changes/<name>/specs/`
- **AND** reads corresponding main specs from `pastelsdd/specs/`
- **AND** reconciles main specs to match what the deltas describe

#### Scenario: Idempotent operation
- **WHEN** agent executes `/pastel:sync` multiple times on the same change
- **THEN** the result is the same as running it once
- **AND** no duplicate requirements are created

#### Scenario: Change selection prompt
- **WHEN** agent executes `/pastel:sync` without specifying a change
...
- **THEN** create new main spec file at `pastelsdd/specs/<capability>/spec.md`
