...
Pastelsdd currently resolves schemas from two locations:
1. User override: `~/.local/share/pastelsdd/schemas/<name>/`
2. Package built-in: `<npm-package>/schemas/<name>/`

This change adds a third, highest-priority level: project-local schemas at `./pastelsdd/schemas/<name>/`.
...
- Schema management CLI commands (`pastelsdd schema copy/which/diff/reset`)
...
```
1. ./pastelsdd/schemas/<name>/              # Project-local (highest)
2. ~/.local/share/pastelsdd/schemas/<name>/ # User override
...
function getProjectSchemasDir(projectRoot: string): string {
  return path.join(projectRoot, 'pastelsdd', 'schemas');
}
```
...
A team could create `pastelsdd/schemas/spec-driven/` that shadows the built-in, causing confusion when someone expects default behavior.

**Mitigation:** The `pastelsdd schemas` command shows the source of each schema. Users can see `spec-driven (project)` vs `spec-driven (package)`.
