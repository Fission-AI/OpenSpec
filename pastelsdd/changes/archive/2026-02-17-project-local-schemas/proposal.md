...
Add project-local schema resolution (`./pastelsdd/schemas/`) as the highest priority in the schema lookup chain. This enables teams to version control custom workflow schemas with their repository.

## Motivation

Currently, schema resolution is 2-level:
1. User override: `~/.local/share/pastelsdd/schemas/<name>/`
...
```
1. ./pastelsdd/schemas/<name>/                    # Project-local (NEW)
2. ~/.local/share/pastelsdd/schemas/<name>/       # User global (XDG)
...
- `pastelsdd/schemas/my-workflow/` → referenced as `my-workflow`
...
- Schema management CLI (`pastelsdd schema copy/which/diff/reset`) - future enhancement
...
mkdir -p pastelsdd/schemas/my-workflow/templates

# Define schema
cat > pastelsdd/schemas/my-workflow/schema.yaml << 'EOF'
name: my-workflow
version: 1
...
# Create templates
echo "# Research\n\n..." > pastelsdd/schemas/my-workflow/templates/research.md
# ... etc
```
...
```bash
# Via CLI flag
pastelsdd new change add-feature --schema my-workflow
pastelsdd status --change add-feature --schema my-workflow
...
```bash
# Commit to repo
git add pastelsdd/schemas/
git commit -m "Add custom workflow schema"
git push

# Team members get it automatically
git pull
pastelsdd status --change add-feature --schema my-workflow  # Just works
```
...
Use existing `findProjectRoot()` pattern or current working directory. The project-local schemas directory is always `./pastelsdd/schemas/` relative to project root.
