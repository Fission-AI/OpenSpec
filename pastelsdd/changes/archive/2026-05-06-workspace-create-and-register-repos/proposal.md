...
Users start workspace work by creating a planning home and linking the repos or folders Pastelsdd should know about.

They should not have to create a change before Pastelsdd can see the relevant repos, monorepo folders, packages, services, or apps.
...
Check what Pastelsdd can resolve and how to fix problems.
```
...
pastelsdd workspace setup --no-interactive --name platform --link /path/to/api --link web=/path/to/web
pastelsdd workspace list
pastelsdd workspace ls
pastelsdd workspace link /path/to/api
pastelsdd workspace link api-service /path/to/api
pastelsdd workspace relink api /new/path/to/api
...
`workspace list` shows known Pastelsdd-managed workspaces from the local workspace registry, including each workspace location and linked repos or folders.
...
Linking should behave like selecting a folder from a picker: Pastelsdd verifies the folder exists, resolves relative inputs to an absolute path in the current runtime, and stores that verified path instead of the raw input string.

When a link name is already in use, Pastelsdd should preserve the existing link and show the conflicting name with the existing path. The error should suggest choosing a different link name, or using `workspace relink <name> <path>` if the user intended to change the existing link's path.
...
Workspace commands should work globally. When a command needs one workspace and the user did not specify it, Pastelsdd should use the local registry to show an interactive picker. In non-interactive mode, it should fail with a clear message and suggest `--workspace <name>`.

When a command runs from inside a valid workspace that is not in the local registry, Pastelsdd should still use that current workspace. It should surface a non-fatal warning status that the workspace is not known locally, and successful mutating commands such as `workspace link` or `workspace relink` should record that workspace in the local registry after they update workspace state.
...
- `workspace doctor` was the right place to answer "what does Pastelsdd know about this workspace?"
...
- The POC required linked repo paths to already contain repo-local `pastelsdd/`. This should become an implementation-readiness signal, not a planning prerequisite.
...
- No public `pastelsdd workspace create` command in this first release.
...
- No requirement that linked repos or folders have repo-local Pastelsdd state.
