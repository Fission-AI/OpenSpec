# Design: Add /pastel:verify Skill

## Architecture Decision: Dynamic Generation via Setup Command

### Context

All existing pastel experimental skills (explore, new, continue, apply, ff, sync, archive) are dynamically generated when users run `pastelsdd artifact-experimental-setup`. They are not manually created files checked into the repository.
...
1. **Consistency**: All 7 existing pastel skills follow this pattern. Adding verify as the 8th skill should follow the same architecture.
...
3. **Distribution**: Users get the verify skill automatically when running `pastelsdd artifact-experimental-setup`, just like all other pastel skills. No special installation steps needed.
...
- Update help text to list `/pastel:verify`

#### 3. Generated Artifacts

When users run `pastelsdd artifact-experimental-setup`, the command creates:

- `.claude/skills/pastelsdd-verify-change/SKILL.md` - Agent Skills format
- `.claude/commands/pastel/verify.md` - Slash command format
...
Create `.claude/skills/pastelsdd-verify-change/SKILL.md` as a static file in the Pastelsdd repository.

**Rejected because:**
- Inconsistent with all other pastel skills
...
Add `pastelsdd setup-verify` as a separate command.
...
3. Help text mentions `/pastel:verify`
4. Running `pastelsdd artifact-experimental-setup` generates both skill and command files
