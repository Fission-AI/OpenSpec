...
Pastelsdd's existing architecture can already represent that shape:

- `AI_TOOLS` can advertise a `skillsDir`
- `init` can install skills for any selected tool with `skillsDir`
- when command generation is attempted for a tool without an adapter, Pastelsdd already records `commandsSkipped`
...
- adding a fake `.kimi/commands/...` path would create behavior Pastelsdd cannot justify against upstream Kimi CLI behavior

### 3. Document Kimi by its real invocation surface

Kimi documentation in Pastelsdd must use Kimi's actual skill invocation form:

- supported-tools: no generated command files, use `/skill:pastelsdd-*`
- commands doc: examples such as `/skill:pastelsdd-propose`

The docs must not claim generated `pastel-*` files or `/pastelsdd-*` direct invocations for Kimi.
