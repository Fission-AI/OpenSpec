...
- Teams already rely on Pastelsdd to manage slash-command configurations for Claude Code, Cursor, OpenCode, Codex, Kilo Code, and Windsurf. Excluding GitHub Copilot forces developers to manually maintain Pastelsdd prompts in `.github/prompts/`, which leads to drift and undermines Pastelsdd's "single source of truth" promise.
- GitHub Copilot discovers prompts from the repository's `.github/prompts/` directory, making it straightforward to version control and share across the team. Adding automated generation and refresh through `pastelsdd init` and `pastelsdd update` eliminates manual synchronization and keeps Pastelsdd instructions consistent across all AI assistants.

## What Changes
- Add GitHub Copilot to the `pastelsdd init` tool picker with "already configured" detection similar to other editors, wiring an implementation that writes managed Markdown prompt files to `.github/prompts/` with Pastelsdd marker blocks.
- Generate three GitHub Copilot prompt files—`pastelsdd-proposal.prompt.md`, `pastelsdd-apply.prompt.md`, and `pastelsdd-archive.prompt.md`—whose content mirrors shared slash-command templates while conforming to Copilot's frontmatter and `$ARGUMENTS` placeholder convention.
- Document GitHub Copilot's repository-based discovery and that Pastelsdd writes prompts to `.github/prompts/` with managed blocks.
- Teach `pastelsdd update` to refresh existing GitHub Copilot prompts in-place (only when they already exist) in the repository's `.github/prompts/` directory.
