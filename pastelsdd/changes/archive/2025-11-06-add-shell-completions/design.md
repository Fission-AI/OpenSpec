...
**Pastelsdd Integration:**
```bash
# After installing: pastelsdd completion install bash
pastelsdd val<TAB>           # Completes to "pastelsdd validate"
pastelsdd validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
pastelsdd show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
**Pastelsdd Integration:**
```zsh
# After installing: pastelsdd completion install zsh
pastelsdd val<TAB>    # Shows menu with "validate" and "view" highlighted
pastelsdd show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
**Pastelsdd Integration:**
```fish
# After installing: pastelsdd completion install fish
pastelsdd val       # Gray suggestion shows "validate" immediately
pastelsdd show a    # Real-time suggestions for changes starting with "a"
pastelsdd <TAB>     # Shows all commands with descriptions in paged menu
```
...
**Pastelsdd Integration:**
```powershell
# After installing: pastelsdd completion install powershell
pastelsdd val<TAB>       # Cycles: validate → view → validate
pastelsdd show <TAB>     # Cycles through change IDs one by one
pastelsdd <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
class CompletionProvider {
  ...
  isPastelsddProject(): boolean {
    // Check for pastelsdd/ directory
  }
}
```
...
- Graceful degradation when outside Pastelsdd projects
...
│       ├── command-registry.ts    # Single source of truth for Pastelsdd commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_pastelsdd
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_pastelsdd
...
2. **Not in Pastelsdd project** - Skip dynamic completions, only offer static commands
...
3. **Manual Testing**
   ...
   - Tab completion behavior in Pastelsdd projects
