...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all Pastelsdd commands, flags, and metadata
...
- [x] Implement installation to `~/.oh-my-zsh/custom/completions/_pastelsdd` for Oh My Zsh
- [x] Implement fallback installation to `~/.zsh/completions/_pastelsdd` with `fpath` updates
...
- [x] Implement dynamic completion helper functions in Zsh generator (`_pastelsdd_complete_changes`, `_pastelsdd_complete_specs`, `_pastelsdd_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real Pastelsdd projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside Pastelsdd projects (should skip dynamic completions)
- [x] Update `pastelsdd --help` output to include completion command (automatically done via Commander)
