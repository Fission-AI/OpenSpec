...
workspace links = the repos or folders Pastelsdd can plan across
...
```bash
pastelsdd workspace open platform
pastelsdd workspace open --workspace platform
```

User-facing docs should prefer the positional form. If both are provided and they differ, Pastelsdd should fail with a clear conflict error.
...
This slice should not add global workspace opener config. Pastelsdd already has a global config system, and workspace-level defaults can be added there later if repeated setup makes the local prompt feel noisy.
...
Store the preferred opener as a structured object in `.pastelsdd-workspace/local.yaml`:
...
```bash
pastelsdd workspace setup --no-interactive --name platform --link /repo --opener codex
pastelsdd workspace setup --no-interactive --name platform --link /repo --opener editor
```
...
If Pastelsdd later supports a Copilot CLI agent, it should use a distinct value such as `github-copilot-cli` and launch the CLI agent directly. VS Code Copilot and a CLI agent have different opener mechanics, so they should remain distinct opener values.
...
For VS Code-style editor support, Pastelsdd should create and maintain a `.code-workspace` file as part of the workspace setup/link/relink lifecycle. `workspace open` should launch against existing workspace state.
...
The `.code-workspace` file should include the workspace root and each linked repo or folder with a valid local path. Because linked paths come from machine-local workspace state, Pastelsdd-created workspaces should ignore the maintained `.code-workspace` file by default.
...
This directory is an Pastelsdd workspace for planning across linked repos or folders.
...
- Use Pastelsdd workspace commands instead of hand-editing `.pastelsdd-workspace/*.yaml`.
```

`workspace open` is a launching feature. It should launch the selected opener against existing workspace files.

For Claude and Codex, `workspace open` may still need to pass workspace and linked directory arguments to the agent process at launch because those tools do not consume `.code-workspace` directly. If an opener requires an initial prompt argument, it should be minimal, such as `Open this Pastelsdd workspace.`

Dynamic workspace facts should normally be discoverable from existing files:

- linked paths: `.pastelsdd-workspace/local.yaml`
- stable link names: `.pastelsdd-workspace/workspace.yaml`
...
Pastelsdd should own a marked workspace-guidance block inside `AGENTS.md`:
...
...
<!-- PASTELSDD:WORKSPACE-GUIDANCE:END -->
```
...
If `AGENTS.md` is missing, Pastelsdd should recreate it. If `AGENTS.md` exists and the markers are absent, Pastelsdd should append the managed block while preserving existing content.

## Linked Paths

Root workspace open should attach every linked repo or folder with a valid local path.

Broken links are skipped during workspace open. Pastelsdd should surface clear status in human output, with `pastelsdd workspace doctor` as the repair path.

Links with repo-local `pastelsdd/` state absent remain valid for workspace open. Missing repo-local Pastelsdd state can matter later for implementation readiness while still allowing visibility for exploration and planning.
