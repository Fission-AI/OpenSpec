...
The system SHALL provide a Nix flake that builds Pastelsdd for multiple platforms.

#### Scenario: Build on Linux x86_64
- **WHEN** user runs `nix build` on x86_64-linux system
- **THEN** system builds Pastelsdd package successfully
- **AND** package includes the `pastelsdd` binary
...
The system SHALL use pnpm for building Pastelsdd in the Nix flake.
...
- **WHEN** user runs `nix develop` in Pastelsdd repository
