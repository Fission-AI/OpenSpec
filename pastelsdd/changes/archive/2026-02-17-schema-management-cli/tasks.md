...
- [x] 3.5 Add validate-all mode when no name provided (scan `pastelsdd/schemas/`)
- [x] 3.6 Add text output with pass/fail indicators and error messages
- [x] 3.7 Add JSON output matching existing `pastelsdd validate` format: `{ valid, issues: [] }`
...
- [x] 5.7 Add `--default` flag to update `pastelsdd/config.yaml` with new schema as default
