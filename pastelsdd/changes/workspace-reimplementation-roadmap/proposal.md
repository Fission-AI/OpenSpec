...
This roadmap also needs to survive multiple sessions and branches. Current Pastelsdd change discovery treats active changes as flat immediate directories under `pastelsdd/changes/`, and change names are kebab-case identifiers rather than nested paths. This change is therefore a flat planning container with sibling proposal changes instead of nested child changes.
...
Add a lightweight roadmap for reimplementing workspace support as a stack of flat sibling Pastelsdd changes:
...
- `workspace-reimplementation-roadmap`: Coordinates the workspace reimplementation plan across multiple flat Pastelsdd changes.

### Modified Capabilities

- `pastelsdd-conventions`: Clarifies that this workspace effort uses flat sibling changes until nested or stacked change metadata is supported.
