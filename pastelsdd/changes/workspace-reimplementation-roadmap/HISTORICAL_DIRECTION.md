...
`pastelsdd/initiatives/context-store-and-initiatives/direction.md`, which locks
this boundary:
...
Fresh-agent entry point: read `pastelsdd/changes/workspace-reimplementation-roadmap/START_HERE.md` first, then return to this document for the full product direction.

This document captures the intended direction for reimplementing Pastelsdd workspace support from scratch, based on what we learned from the workspace POC.
...
The reimplementation should be ordered around the path a real user takes through Pastelsdd:
...
I set up an Pastelsdd workspace.
...
```text
Create a planning home and link the repos or folders Pastelsdd should know about.
```
...
- Use `.pastelsdd-workspace/`, not `.pastelsdd/`, for workspace metadata.
...
- Allow linked repos or folders without repo-local `pastelsdd/` state.
...
Done when a user can set up a workspace, link repos or folders, list known workspaces, relink local paths, and run `doctor` to see exactly what Pastelsdd can resolve.
...
pastelsdd workspace open --agent github-copilot
```
...
```text
Explore how we should make the Pastelsdd docs available on the landing page.
...
- Normal Pastelsdd skills and commands in workspace sessions.
...
```bash
pastelsdd status
pastelsdd status --change integrate-docs
```
...
```text
copy planning files
materialize repo-local Pastelsdd state
create the proposal files for the first time
```

Agent behavior:

1. Ask Pastelsdd for apply context.
...
```json
{
  ...
  "change": "integrate-docs",
  "target": "landing",
  "implementationRoot": "/repos/pastelsdd-landing",
  "contextFiles": [
...
- Worktree management.
- Repo-local Pastelsdd mirroring.
...
- Do not require repo-local planning copies unless Pastelsdd later decides that repo-local archival matters.
