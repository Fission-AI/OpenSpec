...
- **WHEN** user runs `pastelsdd status` without `--change` and no change directories exist under `pastelsdd/changes/`
- **THEN** the CLI prints `No active changes. Create one with: pastelsdd new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `pastelsdd status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `pastelsdd status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `pastelsdd status --change non-existent`
- **THEN** the CLI throws an error with message `Change 'non-existent' not found`

#### Scenario: Other commands unaffected
- **WHEN** user runs `pastelsdd show` or `pastelsdd instructions` without `--change` and no changes exist
