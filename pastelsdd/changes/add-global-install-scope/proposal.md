...
Pastelsdd installation paths are currently inconsistent:
...
- Add install scope control in `pastelsdd config profile` interactive flow.
- Extend `pastelsdd config list` output with install scope source (`explicit`, `new-default`, `legacy-default`).
