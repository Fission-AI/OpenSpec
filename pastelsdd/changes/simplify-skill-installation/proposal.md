...
```text
0:00  $ pastelsdd init
      ✓ Done. Run /pastel:propose "your idea"

0:15  /pastel:propose "add user authentication"
...
Power users can customize via `pastelsdd config profile`.
...
`/pastel:propose` should naturally onboard users by explaining what it's doing:
...
Stored in existing global config (`~/.config/pastelsdd/config.json`). Not prompted during init.
...
pastelsdd config profile core     # preset shortcut (core workflows, preserves delivery)
```
...
**Existing users keep their current setup.** When `pastelsdd update` runs on a project with existing workflows and no `profile` in global config, it performs a one-time migration:
...
- `pastelsdd init` on a **new** project (no existing workflows) uses global config, defaulting to `core`
...
- Migration message mentions `propose` and suggests `pastelsdd config profile core` to opt in
- After migration, users can opt into `core` profile via `pastelsdd config profile core`
...
```json
// ~/.config/pastelsdd/config.json (extends existing)
{
  "telemetry": { ... },          // existing
...
| custom | user-defined | Pick exactly what you need via `pastelsdd config profile` |
