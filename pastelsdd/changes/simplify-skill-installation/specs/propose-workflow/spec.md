...
The propose workflow SHALL combine change creation and artifact generation into a single command, reducing friction for new users while teaching them the Pastelsdd workflow through embedded guidance.
...
- **WHEN** user invokes `/pastel:propose "add user authentication"`
- **THEN** the system SHALL create a change directory with kebab-case name
- **THEN** the system SHALL create `.pastelsdd.yaml` in the change directory (via `pastelsdd new change`)
...
- **WHEN** user invokes `/pastel:propose` with a name that already exists
...
- **WHEN** user invokes `/pastel:propose`
- **THEN** the system SHALL explain what artifacts will be created (proposal.md, design.md, specs/, tasks.md)
- **THEN** the system SHALL indicate next step (`/pastel:apply` to implement)
...
- **WHEN** user invokes `/pastel:propose "feature name"`
- **THEN** the result SHALL be functionally equivalent to invoking `/pastel:new "feature-name"` followed by `/pastel:ff feature-name`
