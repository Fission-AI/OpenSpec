...
## Purpose
Define `pastelsdd feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `pastelsdd feedback` command that creates a GitHub Issue in the pastelsdd repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `pastelsdd feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the pastelsdd repository
...
- **WHEN** user executes `pastelsdd feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (Pastelsdd version, platform, timestamp)
...
- **WHEN** user runs `pastelsdd feedback "message"`
...
- **THEN** the issue body includes:
  - Pastelsdd CLI version
  - Platform (darwin, linux, win32)
  - Submission timestamp
  - Separator line: "---\nSubmitted via Pastelsdd CLI"
...
- **WHEN** user has disabled telemetry via `PASTELSDD_TELEMETRY=0`
- **AND** user runs `pastelsdd feedback "message"`
...
- **AND** submits via `pastelsdd feedback` command on user confirmation
...
- **WHEN** user types `pastelsdd fee<TAB>`
- **THEN** the shell completes to `pastelsdd feedback`

#### Scenario: Flag completion

- **WHEN** user types `pastelsdd feedback "msg" --<TAB>`
