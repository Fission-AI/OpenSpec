...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global Pastelsdd configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `pastelsdd config --help`
...
- **WHEN** user executes `pastelsdd config path`
...
- **WHEN** user executes `pastelsdd config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `pastelsdd config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `pastelsdd config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `pastelsdd config get <key>` where the value is an object
...
- **WHEN** user executes `pastelsdd config set <key> <value>`
...
- **WHEN** user executes `pastelsdd config set <key> true` or `pastelsdd config set <key> false`
...
- **WHEN** user executes `pastelsdd config set <key> <value> --string`
...
- **WHEN** user executes `pastelsdd config set featureFlags.newFlag true`
...
- **WHEN** user executes `pastelsdd config unset <key>`
...
- **WHEN** user executes `pastelsdd config reset --all`
...
- **WHEN** user executes `pastelsdd config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `pastelsdd config reset` without `--all`
...
- **WHEN** user executes `pastelsdd config edit`
...
The `pastelsdd config profile` command SHALL provide an action-first interactive flow that allows users to modify delivery and workflow settings independently.

#### Scenario: Current profile summary appears first

- **WHEN** user runs `pastelsdd config profile` in an interactive terminal
...
- **WHEN** user runs `pastelsdd config profile` interactively
...
- **WHEN** delivery selection is shown in `pastelsdd config profile`
...
- **WHEN** `pastelsdd config profile` exits with `No config changes.` inside an Pastelsdd project
...
- **AND** include guidance to run `pastelsdd update` to sync project files

#### Scenario: Apply prompt is gated on actual changes

- **WHEN** config values were changed and saved
- **AND** current directory is an Pastelsdd project
- **THEN** prompt `Apply changes to this project now?`
- **AND** if confirmed, run `pastelsdd update` for the current project
...
- **WHEN** user executes `pastelsdd config set someFutureKey 123`
...
- **WHEN** user executes `pastelsdd config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `pastelsdd config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `pastelsdd config --scope project <subcommand>`
