...
The system SHALL resolve schemas from the project-local directory (`./pastelsdd/schemas/<name>/`) with highest priority when a `projectRoot` is provided.

#### Scenario: Project-local schema takes precedence over user override
- **WHEN** a schema named "my-workflow" exists at `./pastelsdd/schemas/my-workflow/schema.yaml`
- **AND** a schema named "my-workflow" exists at `~/.local/share/pastelsdd/schemas/my-workflow/schema.yaml`
...
- **WHEN** a schema named "spec-driven" exists at `./pastelsdd/schemas/spec-driven/schema.yaml`
...
- **WHEN** no schema named "my-workflow" exists at `./pastelsdd/schemas/my-workflow/`
- **AND** a schema named "my-workflow" exists at `~/.local/share/pastelsdd/schemas/my-workflow/schema.yaml`
- **AND** `getSchemaDir("my-workflow", projectRoot)` is called
...
- **WHEN** no schema named "spec-driven" exists at `./pastelsdd/schemas/spec-driven/`
- **AND** no schema named "spec-driven" exists at `~/.local/share/pastelsdd/schemas/spec-driven/`
- **AND** "spec-driven" is a package built-in schema
- **AND** `getSchemaDir("spec-driven", projectRoot)` is called
...
- **THEN** the system SHALL return `/path/to/project/pastelsdd/schemas`
...
- **WHEN** a schema named "team-flow" exists at `./pastelsdd/schemas/team-flow/schema.yaml`
...
- **WHEN** a schema named "my-custom" exists only at `~/.local/share/pastelsdd/schemas/my-custom/`
- **AND** `listSchemasWithInfo(projectRoot)` is called
...
The `pastelsdd schemas` command SHALL display the source of each schema.

#### Scenario: Display format includes source
- **WHEN** user runs `pastelsdd schemas`
...
The system SHALL use the schema field from `pastelsdd/config.yaml` as the default when creating new changes without explicit `--schema` flag and no planning-home default applies.

#### Scenario: Create change without --schema flag and config exists
- **WHEN** user runs `pastelsdd new change foo`, no planning-home default applies, and config contains `schema: "tdd"`
- **THEN** system creates change with schema "tdd"

#### Scenario: Create change without --schema flag and no config
- **WHEN** user runs `pastelsdd new change foo`, no planning-home default applies, and no config file exists
- **THEN** system creates change with default schema "spec-driven"

#### Scenario: Create change with explicit --schema flag
- **WHEN** user runs `pastelsdd new change foo --schema custom` and config contains `schema: "tdd"`
...
- **WHEN** change has `.pastelsdd.yaml` with `schema: bound` and config has `schema: tdd`
...
The system SHALL allow the config schema field to reference project-local schemas defined in `pastelsdd/schemas/`.

#### Scenario: Config references project-local schema
- **WHEN** config contains `schema: "my-workflow"` and `pastelsdd/schemas/my-workflow/` exists
...
- **THEN** error message includes "Fix: Edit pastelsdd/config.yaml and change 'schema: X' to a valid schema name"
...
- **THEN** existing changes continue to use their bound schema from `.pastelsdd.yaml`
...
- **WHEN** a user runs `pastelsdd schemas`
...
- **WHEN** Pastelsdd resolves the schema for the new change
...
- **THEN** Pastelsdd SHALL use the explicitly requested schema
