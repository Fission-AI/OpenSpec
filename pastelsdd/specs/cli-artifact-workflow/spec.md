...
- **WHEN** user runs `pastelsdd status --change <id>`
...
- **WHEN** user runs `pastelsdd status --change <id> --json`
...
- **WHEN** user runs `pastelsdd status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `pastelsdd status` without `--change`
...
- **WHEN** user runs `pastelsdd status --change unknown-id`
- **AND** directory `pastelsdd/changes/unknown-id/` does not exist
- **THEN** the system displays an error listing all available change directories

### Requirement: Next Artifact Discovery

The workflow SHALL use `pastelsdd status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `pastelsdd status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `pastelsdd instructions <artifact> --change <id>`
...
- **WHEN** user runs `pastelsdd instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `pastelsdd instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `pastelsdd instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `pastelsdd templates`
...
- **WHEN** user runs `pastelsdd new change add-feature`
- **THEN** the system creates `pastelsdd/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `pastelsdd new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `pastelsdd new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `pastelsdd new change add-feature --description "Add new feature"`
...
- **THEN** the CLI SHALL support `pastelsdd workspace ls`
- **AND** it SHALL behave the same as `pastelsdd workspace list`
...
- **THEN** the CLI SHALL use `pastelsdd workspace setup`
- **AND** it SHALL not expose `pastelsdd workspace create` as the public creation path
...
- **WHEN** user runs `pastelsdd status --change <id> --schema tdd`
...
The system SHALL generate schema-aware apply instructions via `pastelsdd instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `pastelsdd instructions apply --change <id>`
...
- **WHEN** user runs `pastelsdd instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
- **WHEN** user runs `pastelsdd artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `pastelsdd artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `pastelsdd artifact-experimental-setup --tool unknown-tool`
...
- **WHEN** a user runs `pastelsdd status --change <id> --json`
...
- **AND** agents SHALL be able to read those paths without assuming `pastelsdd/changes/<id>/`
...
- **GIVEN** the change is workspace-scoped
- **WHEN** a user runs `pastelsdd instructions <artifact> --change <id> --json`
...
Generated workflow skills SHALL use Pastelsdd CLI output as the source of truth for artifact locations.
...
- **THEN** it SHALL instruct the agent to run `pastelsdd status --change <id> --json`
...
- **THEN** it SHALL instruct the agent to run `pastelsdd instructions <artifact> --change <id> --json`
...
- **THEN** they SHALL avoid hardcoded examples that require changes to live under `pastelsdd/changes/<id>/`
