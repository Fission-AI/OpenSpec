# Pastelsdd Conventions Specification

## Purpose

Pastelsdd conventions SHALL define how system capabilities are documented, how changes are proposed and tracked, and how specifications evolve over time. This meta-specification serves as the source of truth for Pastelsdd's own conventions.
## Requirements
### Requirement: Structured conventions for specs and changes

Pastelsdd conventions SHALL mandate a structured spec format with clear requirement and scenario sections so tooling can parse consistently.

#### Scenario: Following the structured spec format

- **WHEN** writing or updating Pastelsdd specifications
...
Pastelsdd specifications SHALL capture verifiable behavior contracts and avoid internal implementation detail.
...
Pastelsdd conventions SHALL keep specs lightweight by default and scale rigor only when risk or coordination complexity demands it.
...
An Pastelsdd project SHALL maintain a consistent directory structure for specifications and changes.

#### Scenario: Initializing project structure
- **WHEN** an Pastelsdd project is initialized
...
Pastelsdd CLI design SHALL use verbs as top-level commands with nouns provided as arguments or flags for scoping.

#### Scenario: Verb-first command discovery
- **WHEN** a user runs a command like `pastelsdd list`
...
- **WHEN** users run noun-prefixed commands such as `pastelsdd spec ...` or `pastelsdd change ...`
...
- **THEN** `pastelsdd show` and `pastelsdd validate` SHALL accept `--type spec|change`
...
Pastelsdd conventions SHALL distinguish workspace planning concepts using user-facing product language.
...
Pastelsdd conventions SHALL distinguish workspace-level planning from repo-local implementation ownership.
...
- **WHEN** workspace guidance describes Pastelsdd workflows
