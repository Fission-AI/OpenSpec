...
## Purpose
Define `pastelsdd schema init` behavior for creating project-local schema skeletons in interactive and non-interactive modes.

## Requirements
### Requirement: Schema init command creates project-local schema
The CLI SHALL provide an `pastelsdd schema init <name>` command that creates a new schema directory under `pastelsdd/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `pastelsdd schema init my-workflow`
- **THEN** system creates directory `pastelsdd/schemas/my-workflow/`
...
- **WHEN** user runs `pastelsdd schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `pastelsdd schema init existing-schema` and `pastelsdd/schemas/existing-schema/` already exists
...
- **WHEN** user runs `pastelsdd schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `pastelsdd schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `pastelsdd schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates `pastelsdd/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Set as default via flag
- **WHEN** user runs `pastelsdd schema init my-workflow --default`
- **THEN** system creates schema and updates `pastelsdd/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Skip setting default
- **WHEN** user runs `pastelsdd schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `pastelsdd/config.yaml`
...
- **WHEN** user runs `pastelsdd schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `pastelsdd schema init "invalid name" --json`
