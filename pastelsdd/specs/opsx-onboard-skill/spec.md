# pastel-onboard-skill Specification

## Purpose
Define `/pastel:onboard` behavior for guiding users through an end-to-end Pastelsdd workflow on their real codebase.

## Requirements
### Requirement: OPSX Onboard Skill

The system SHALL provide an `/pastel:onboard` skill that guides users through their first complete Pastelsdd workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/pastel:onboard`
- **THEN** agent checks if Pastelsdd is initialized
- **AND** if not initialized, prompts user to run `pastelsdd init` first
...
- **THEN** agent briefly demonstrates `/pastel:explore` by investigating relevant code
...
- **THEN** agent runs `pastelsdd new change "<name>"` with derived kebab-case name
...
- **AND** provides command reference table (/pastel:explore, /pastel:new, /pastel:ff, /pastel:continue, /pastel:apply, /pastel:verify, /pastel:archive)
- **AND** suggests next actions (try /pastel:new or /pastel:ff on something)
...
- **AND** explains how to continue later with `/pastel:continue <name>`
...
- **AND** exits gracefully with encouragement to try `/pastel:new`
