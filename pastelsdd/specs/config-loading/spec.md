...
## Purpose
Define how `pastelsdd/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from pastelsdd/config.yaml

The system SHALL read and parse the project configuration file located at `pastelsdd/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `pastelsdd/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `pastelsdd/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `pastelsdd/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `pastelsdd/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `pastelsdd/config.yml` exists and `pastelsdd/config.yaml` does not exist
- **THEN** system reads from `pastelsdd/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `pastelsdd/config.yaml` and `pastelsdd/config.yml` exist
- **THEN** system prefers `pastelsdd/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `pastelsdd new change`
