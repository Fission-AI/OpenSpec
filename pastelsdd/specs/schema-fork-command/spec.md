...
## Purpose
Define `pastelsdd schema fork` behavior for cloning existing schemas into project-local schemas with safe overwrite controls.

## Requirements
### Requirement: Schema fork copies existing schema
The CLI SHALL provide an `pastelsdd schema fork <source> [name]` command that copies an existing schema to the project's `pastelsdd/schemas/` directory.

#### Scenario: Fork with explicit name
- **WHEN** user runs `pastelsdd schema fork spec-driven my-custom`
- **THEN** system locates `spec-driven` schema using resolution order (project → user → package)
- **AND** copies all files to `pastelsdd/schemas/my-custom/`
...
- **WHEN** user runs `pastelsdd schema fork spec-driven` without specifying a name
- **THEN** system copies to `pastelsdd/schemas/spec-driven-custom/`
- **AND** updates `name` field in `schema.yaml` to `spec-driven-custom`

#### Scenario: Source schema not found
- **WHEN** user runs `pastelsdd schema fork nonexistent`
...
- **WHEN** user runs `pastelsdd schema fork spec-driven my-custom` and `pastelsdd/schemas/my-custom/` exists
...
- **WHEN** user runs `pastelsdd schema fork spec-driven my-custom --force` and destination exists
...
- **WHEN** user runs `pastelsdd schema fork spec-driven my-custom` in interactive mode and destination exists
...
- **WHEN** user runs `pastelsdd schema fork spec-driven my-custom --json`
...
- **WHEN** user runs `pastelsdd schema fork spec-driven --json`
