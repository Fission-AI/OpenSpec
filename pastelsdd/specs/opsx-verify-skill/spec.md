# pastel-verify-skill Specification

## Purpose
Define `/pastel:verify` behavior for assessing implementation completeness, correctness, and coherence against change artifacts.

## Requirements
### Requirement: Verify Skill Invocation
The system SHALL provide an `/pastel:verify` skill that validates implementation against change artifacts.

#### Scenario: Verify with change name provided
- **WHEN** agent executes `/pastel:verify <change-name>`
...
- **WHEN** agent executes `/pastel:verify` without a change name
...
- **AND** suggests running `/pastel:continue` to create tasks
...
- **WHEN** verifying completeness
- **AND** delta specs exist in `pastelsdd/changes/<name>/specs/`
