...
## Purpose
Define how Pastelsdd opens a workspace working set through a selected agent or
...
Pastelsdd SHALL provide a `workspace open` command that opens an Pastelsdd workspace working set through an agent or VS Code editor.

#### Scenario: Opening the current workspace
- **GIVEN** the command runs from inside an Pastelsdd workspace
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL open that current workspace
...
- **WHEN** the user runs `pastelsdd workspace open platform`
- **THEN** Pastelsdd SHALL open the `platform` workspace

#### Scenario: Opening a named workspace with the selection flag
- **GIVEN** a workspace named `platform` is known locally
- **WHEN** the user runs `pastelsdd workspace open --workspace platform`
- **THEN** Pastelsdd SHALL open the `platform` workspace

#### Scenario: Conflicting workspace selectors
- **GIVEN** workspaces named `platform` and `checkout` are known locally
- **WHEN** the user runs `pastelsdd workspace open platform --workspace checkout`
- **THEN** Pastelsdd SHALL fail with a clear conflict error
- **AND** the error SHALL name both conflicting selectors

#### Scenario: Handling unsupported preview and JSON flags
- **WHEN** the user runs `pastelsdd workspace open` with `--prepare-only` or `--json`
- **THEN** Pastelsdd SHALL fail with a clear error that the root workspace open surface supports launching through a selected opener
...
- **WHEN** the user runs `pastelsdd workspace open --change <id>`
- **THEN** Pastelsdd SHALL fail with a clear error that this slice supports root workspace open
...
Pastelsdd SHALL resolve the workspace to open using current workspace context, local registry state, and interactive selection.
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL open the current workspace
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL open that known workspace directly
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL present a picker with workspace names and locations
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL fail with a clear message listing the known workspace names
...
- **AND** it SHALL suggest running `pastelsdd workspace setup`

### Requirement: Opener Resolution
Pastelsdd SHALL resolve the opener from command overrides, workspace-local preference, or an interactive prompt.

#### Scenario: Conflicting opener overrides
- **WHEN** the user runs `pastelsdd workspace open --agent codex --editor`
- **THEN** Pastelsdd SHALL fail with a clear conflict error naming `--agent` and `--editor`
...
- **WHEN** the user runs `pastelsdd workspace open` using default opener resolution
- **THEN** Pastelsdd SHALL use the stored preferred opener

#### Scenario: Overriding with an agent for one session
- **GIVEN** the workspace has a stored preferred opener
- **WHEN** the user runs `pastelsdd workspace open --agent codex`
- **THEN** Pastelsdd SHALL use Codex for that open command
- **AND** it SHALL leave the stored preferred opener unchanged

#### Scenario: Overriding with VS Code editor for one session
- **GIVEN** the workspace has a stored preferred opener
- **WHEN** the user runs `pastelsdd workspace open --editor`
- **THEN** Pastelsdd SHALL open the workspace in VS Code editor mode
- **AND** it SHALL leave the stored preferred opener unchanged
...
- **AND** the terminal is interactive
- **WHEN** the user runs `pastelsdd workspace open` using default opener resolution
- **THEN** Pastelsdd SHALL prompt the user to choose an opener
...
- **WHEN** the user runs `pastelsdd workspace open` using default opener resolution
- **THEN** Pastelsdd SHALL fail with a clear message that no supported opener is available
...
Pastelsdd SHALL launch the selected opener using existing workspace files and linked path state.

#### Scenario: Opening VS Code editor
- **GIVEN** the user selected the VS Code editor opener
- **WHEN** `code` is available on `PATH`
- **THEN** Pastelsdd SHALL open the workspace's maintained `.code-workspace` file with VS Code
...
- **THEN** Pastelsdd SHALL launch Codex from the workspace root
...
- **THEN** Pastelsdd SHALL launch Claude from the workspace root
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL fail with a clear error naming the missing executable
...
- **WHEN** the user runs `pastelsdd workspace open`
- **THEN** Pastelsdd SHALL fail with a clear error naming `code`
...
Pastelsdd SHALL make linked repos and folders visible for workspace exploration and planning before change creation.
...
- **THEN** Pastelsdd SHALL include every valid linked path in the opened working set
...
- **THEN** Pastelsdd SHALL skip the broken linked path
- **AND** it SHALL report that the path was skipped with `pastelsdd workspace doctor` as the repair path
- **AND** it SHALL continue opening the workspace when the selected opener itself is available

#### Scenario: Opening links with repo-local Pastelsdd state absent
- **GIVEN** a linked repo or folder has a valid local path and repo-local `pastelsdd/` state is absent
- **WHEN** the user opens the workspace
- **THEN** Pastelsdd SHALL include that link when its local path is valid
- **AND** it SHALL treat missing repo-local Pastelsdd state as an implementation-readiness concern for later workflows while continuing open

### Requirement: Workspace Open Guidance
Pastelsdd SHALL use durable workspace guidance as the primary context source for root workspace open.

#### Scenario: Launching with existing workspace guidance
- **GIVEN** the workspace has Pastelsdd-managed guidance in `AGENTS.md`
- **WHEN** the user opens the workspace
- **THEN** Pastelsdd SHALL refresh the maintained `.code-workspace` from current linked path state
...
- **WHEN** Pastelsdd launches that opener
- **THEN** Pastelsdd SHALL use a minimal prompt such as `Open this Pastelsdd workspace.`
