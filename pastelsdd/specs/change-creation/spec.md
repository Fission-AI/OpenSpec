...
## Purpose
Provide programmatic utilities for creating and validating Pastelsdd change directories.
## Requirements
...
- **THEN** the system creates `pastelsdd/changes/add-auth/` directory

#### Scenario: Duplicate change rejected
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `pastelsdd/changes/add-auth/` already exists
- **THEN** the system throws an error indicating the change already exists

#### Scenario: Creates parent directories if needed
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `pastelsdd/changes/` does not exist
...
- **AND** it SHALL not create the change under a linked repo's `pastelsdd/changes/` directory
...
- **WHEN** the user creates a new change
- **THEN** Pastelsdd SHALL resolve the current workspace as the planning home
...
- **THEN** Pastelsdd SHALL preserve repo-local change creation behavior for that location
...
- **GIVEN** the command runs outside an Pastelsdd workspace
- **WHEN** the user creates a new change in a repo-local Pastelsdd project
- **THEN** Pastelsdd SHALL continue to create the change under `pastelsdd/changes/`
...
- **THEN** Pastelsdd SHALL reject those invalid affected areas
...
- **THEN** Pastelsdd SHALL create the workspace change
