...
## Purpose
Define how Pastelsdd creates, tracks, and guides workspace-level changes whose planning artifacts coordinate multiple linked repos or folders before implementation ownership is finalized.

## Requirements
### Requirement: Workspace change planning home
Pastelsdd SHALL support workspace-level changes whose shared plan lives in the workspace planning home.

#### Scenario: Creating a workspace change
- **GIVEN** the command runs from an Pastelsdd workspace
- **WHEN** the user creates a change for workspace planning
- **THEN** Pastelsdd SHALL create the change under the workspace planning path
...
- **WHEN** Pastelsdd reports or creates planning artifacts for that change
...
- **THEN** Pastelsdd SHALL capture the product goal at the workspace change level
...
- **WHEN** Pastelsdd creates a workspace-level change
- **THEN** it SHALL not create repo-local Pastelsdd change directories inside linked repos or folders
- **AND** it SHALL not edit implementation files in linked repos or folders

### Requirement: Workspace affected areas
Pastelsdd SHALL represent ownership or implementation boundaries in a workspace change as affected areas.
...
- **THEN** Pastelsdd SHALL validate those area names against the workspace links
...
- **THEN** Pastelsdd SHALL allow the shared plan to exist before all affected areas are finalized
...
- **WHEN** Pastelsdd reports or creates workspace-scoped specs
...
- **THEN** Pastelsdd SHALL distinguish affected areas from delivery slices or phases
...
Pastelsdd SHALL keep the workspace change plan as the source of truth until implementation begins for a selected affected area.
...
- **THEN** Pastelsdd SHALL require an explicit implementation workflow with a selected affected area
