...
The initiative link should live in the repo-local change `.pastelsdd.yaml`.
...
- `createChange()` already writes `.pastelsdd.yaml` for every change.
...
Initiative-linked creation should use `pastelsdd new change` with `--initiative`.

Supported first-slice forms:

```bash
pastelsdd new change add-billing-api --initiative billing-launch --json
pastelsdd new change add-billing-api --initiative platform/billing-launch --json
pastelsdd new change add-billing-api --initiative billing-launch --store platform --json
```
...
```bash
pastelsdd new change add-billing-api --initiative platform/billing-launch
pastelsdd new change add-billing-api --initiative billing-launch --store platform
pastelsdd new change add-billing-api --initiative billing-launch --store-path ./context
```
...
If `pastelsdd new change <id> --initiative ...` runs from a workspace planning
...
```json
{
  "change": {
    "id": "add-billing-api",
    "path": "/repo/pastelsdd/changes/add-billing-api",
    "metadataPath": "/repo/pastelsdd/changes/add-billing-api/.pastelsdd.yaml",
    "schema": "spec-driven"
  },
  "initiative": {
    "store": "platform",
...
```bash
pastelsdd set change add-billing-api --initiative billing-launch --json
pastelsdd set change add-billing-api --initiative platform/billing-launch --json
pastelsdd set change add-billing-api --initiative billing-launch --store platform --json
pastelsdd set change add-billing-api --initiative billing-launch --store-path ../context --json
```
...
file it may mutate is `pastelsdd/changes/<id>/.pastelsdd.yaml`.
...
- `set change` matches the actual side effect: writing validated change metadata
  to `.pastelsdd.yaml`.
- Keeping the command scoped to `.pastelsdd.yaml` avoids creating a broad change
  editing surface.
- `pastelsdd change ...` is currently deprecated, `edit` implies opening an
  editor, and `update` already means refreshing local Pastelsdd tooling or
  guidance.
...
- `pastelsdd new change <id> --initiative ...` for repo-local changes, with
  `--json`, `--store`, and `--store-path` support.
- `pastelsdd set change <id> --initiative ...` for existing repo-local changes.
