...
```text
Pastelsdd initiative: Billing Launch
...
- Not found: say the initiative was not found in registered context stores and
  suggest `pastelsdd initiative list`.
- Ambiguous: show matching stores and paths, then suggest
  `pastelsdd initiative show <id> --store <store>`.
- Incomplete lookup: say some context stores could not be read, include partial
  matches when present, then suggest `pastelsdd context-store doctor`.
...
- Top-level `pastelsdd show` integration.
