...
- Older command surfaces still assume `pastelsdd/changes/` under a local
  Pastelsdd project and need an explicit audit before any implementation slice.
...
Pastelsdd should keep that separation while making the agent handoff sharper
...
briefs, or proposals, not executable Pastelsdd changes, until Item 13 defines a
...
- Use `initiative show` plus workspace/repo context as the agent handoff
  backbone.
- Do not implement context-store-backed Pastelsdd changes in Item 8.
...
- The sharper product question is whether initiative-hosted artifacts can
  become executable Pastelsdd changes after they are bound to a target repo or
  spec root.
...
```text
Plan lives in: repo-local Pastelsdd | initiative context
...
- `src/commands/validate.ts` validates changes and specs from
  `process.cwd()/pastelsdd/...`.
- `src/core/archive.ts` archives by reading `pastelsdd/changes`, applying deltas
  to `pastelsdd/specs`, and moving the change into `pastelsdd/changes/archive`.
...
- Generated skills and workflow templates still contain repo-local path
  assumptions such as `pastelsdd/changes/<name>/`.
