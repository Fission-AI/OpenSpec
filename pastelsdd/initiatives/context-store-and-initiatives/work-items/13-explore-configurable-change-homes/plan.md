...
initiative artifacts can become executable, target-bound Pastelsdd changes.
...
The current initiative direction assumes Pastelsdd changes usually live in the
...
Decide whether Pastelsdd should support initiative-hosted artifacts that can
...
Repo-local change
  -> home: repo/pastelsdd/changes/<id>/
...
output should prefer clearer phrases like "plan lives in repo-local Pastelsdd",
...
- A targetless initiative artifact is a brief, work item, or proposal, not an
  implementation-ready Pastelsdd change.
...
- How does local resolution map a target repo identity to a checkout path,
  Pastelsdd root, branch, and allowed edit roots?
...
- How does `pastelsdd new change` report work target, artifact home,
...
- Should archive for an initiative-hosted target-bound change archive centrally,
  ...
  exists?
- Which command and skill surfaces still hardcode `pastelsdd/changes/`, current
...
Do not implement initiative-hosted executable changes until Pastelsdd has one
