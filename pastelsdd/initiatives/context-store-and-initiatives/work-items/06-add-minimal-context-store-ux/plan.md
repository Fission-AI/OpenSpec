...
- Portable store metadata is exactly `.pastelsdd-store/store.yaml`. It should be
...
```bash
pastelsdd context-store setup <id> [--path <path>] [--init-git|--no-init-git] [--json]
pastelsdd context-store register <path> [--id <id>] [--json]
pastelsdd context-store list [--json]
pastelsdd context-store doctor [id] [--json]
pastelsdd initiative list [--store <id>] [--store-path <path>] [--json]
```
...
- File paths fail.
- `.pastelsdd-store/store.yaml` is written when missing.
...
Metadata: /Users/me/work/team-context/.pastelsdd-store/store.yaml
Registry: /Users/me/.local/share/pastelsdd/context-stores/registry.yaml
Git: initialized
```
...
- If `.pastelsdd-store/store.yaml` exists, use its id.
...
Metadata: /Users/me/src/team-context/.pastelsdd-store/store.yaml
Registry: /Users/me/.local/share/pastelsdd/context-stores/registry.yaml
```
...
Next:
  ...
  pastelsdd context-store register /path/to/context-store
```
...
```text
Pastelsdd initiatives (3 across 2 stores)
...
Run: pastelsdd context-store doctor
```
...
```text
Pastelsdd initiatives in platform (2)
...
- Added `pastelsdd context-store setup/register/list/doctor`.
