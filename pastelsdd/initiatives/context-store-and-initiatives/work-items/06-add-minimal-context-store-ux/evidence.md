...
- A store can start as a local Git-backed folder. Pastelsdd can help create the
...
- Portable `.pastelsdd-store/store.yaml` metadata should be checked in and should
  not include local paths.
- `.pastelsdd-store/store.yaml` is the identity file itself, not a bundle beside
