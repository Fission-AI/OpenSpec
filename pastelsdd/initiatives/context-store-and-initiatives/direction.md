...
The user should not need to know every command. Pastelsdd should expose small,
...
- create or link a repo-local Pastelsdd change
...
of initiative prose. If an agent needs a compact context pack, Pastelsdd can
...
```text
Pastelsdd stores agreed implementation context.
Jira tracks work.
...
An initiative can span one or many Pastelsdd changes.

Those changes may live:

- in the same repo as the initiative
- in different repos
- in multiple context stores or Pastelsdd roots later
...
repo and create a linked Pastelsdd change.
...
Product/program/architect creates initiative
  ...
  -> repo team creates linked Pastelsdd change
...
const store = createStore({
  ...
  backend: gitBackend({
    ...
    localPath: "~/.pastelsdd/stores/acme-context",
    autoSync: true,
  }),
  collections: [
...
- Where should enterprise context stores live by default: customer GitHub,
  Pastelsdd-managed Git, or later hosted cloud?
