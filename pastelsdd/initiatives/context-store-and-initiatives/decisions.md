...
`pastelsdd/initiatives/context-store-and-initiatives/` rather than creating an
Pastelsdd change for each roadmap item.

Why: The initiative is the durable coordination object for this work. Repo-local
Pastelsdd changes should be reserved for implementation slices owned by a repo or
...
- Link repo-local Pastelsdd changes back to the initiative later when
...
Decision: Do not use the initial direction lock to rewrite Pastelsdd specs.
...
cross-team or cross-repo intent, repo-local Pastelsdd changes own implementation
...
proposal"; Pastelsdd should provide small CLI primitives the agent can compose.
