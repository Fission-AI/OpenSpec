...
- `/pastel:apply` just reads whatever's on disk
- `/pastel:continue` only creates the NEXT artifact - won't touch existing ones

If you edit `design.md` after `tasks.md` exists, your only options are:
1. Delete tasks.md manually, then run `/pastel:continue`
...
Add artifact staleness detection to `/pastel:apply`:

1. **Track modification times**: When generating an artifact, record the mtime of its dependencies
2. **Detect staleness**: When `/pastel:apply` runs, check if upstream artifacts (design.md, specs) have been modified since tasks.md was generated
3. **Prompt user**: If stale, ask: "Design was modified after tasks were generated. Would you like to regenerate tasks with `/pastel:continue`?"
...
/pastel:continue --regenerate tasks

# Option B: Separate command
/pastel:regenerate tasks
...
Store `.pastelsdd-meta.json` in change directory:
```json
{
  "tasks.md": {
