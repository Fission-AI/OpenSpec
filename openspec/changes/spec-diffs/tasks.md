...
- [ ] 1.1 Install the `diff` npm package: `pnpm add diff` and `pnpm add -D @types/diff`
...
- [ ] 3.1 In `src/utils/requirement-block.ts`, add function `diffRequirementBlock(baseBlock: string | null, deltaBlock: string, label: string): string` that uses `createPatch` from the `diff` package to produce a unified diff. When `baseBlock` is null, diff against empty string (all additions).
...
- [ ] 3.4 Add unit tests for `buildRenameMap`: single rename, multiple renames, empty list
...
- [ ] 5.3 Handle the no-delta-specs case: print a message like "No delta specs found for change '<name>'" and return (exit code 0)
