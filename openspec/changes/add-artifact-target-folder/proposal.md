...
- Artifacts with a `folder:` set are **never archived**. They persist in their target location after the change ships, and version control is the source of history (no delta format, no archival sweep).
...
- Schema validation rejects `folder:` values that are absolute, contain `..` segments that escape the repo root, or collide with the reserved `openspec/` prefix.
...
- `artifact-graph`: extend the artifact schema to accept an optional `folder:` field; surface it on the loaded `ArtifactGraph` model so downstream consumers (workflow, archive, validate) can branch on it.
...
- `cli-archive`: archival skips artifacts whose definition declares a `folder:` — they remain in place when the change is archived.
- `schema-validate-command`: validate `folder:` values (relative path, no `..` escape, not under `openspec/`).

## Impact

- **Schema YAML**: new optional field on artifacts; built-in `spec-driven` schema is unaffected.
- **Code**: schema Zod type, `ArtifactGraph` loader, instruction-writer path resolution, archive selector, schema validator.
