...
`workspace-create-and-register-repos` creates the workspace and makes working-set entries visible before a change exists. Working-set entries may be repos, monorepo modules, or planning-only code areas. This preserves the product rule that workspace visibility is not change commitment.

`workspace-open-agent-context` gives the agent the workspace root, working-set entries, active changes, and selected change scope.
