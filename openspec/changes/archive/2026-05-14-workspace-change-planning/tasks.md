...
User-testable outcome: A user can run workspace setup, choose which agents get the active profile's Pastelsdd skills, and verify the selected skills are generated in the workspace root only.

- [x] 1.1 Add an interactive workspace setup step named "Install agent skills" that asks which agents should get Pastelsdd skills in this workspace.
...
- [x] 1.6 Ensure `pastelsdd workspace setup` generates or refreshes Pastelsdd agent skills in the workspace root for the selected agents.
- [x] 1.7 Keep setup-time skill generation scoped to the workspace planning home; do not write skills or Pastelsdd artifacts into linked repos or folders during workspace setup.
...
- [x] 2.1 Add a workspace update flow that refreshes, adds, or removes Pastelsdd agent skills in an existing workspace.
- [x] 2.2 Let `pastelsdd workspace update` resolve the current workspace when run from inside a workspace.
- [x] 2.3 Support named and selected-workspace update forms such as `pastelsdd workspace update platform` and `pastelsdd workspace update --workspace platform`.
- [x] 2.4 Support non-interactive update forms such as `pastelsdd workspace update platform --tools codex,claude`.
- [x] 2.5 Remove only known Pastelsdd-managed workflow skill directories for agents that are no longer selected.
...
- [x] 2.10 Detect workspace-local skill drift from the active global profile and report `pastelsdd workspace update` guidance.
...
User-testable outcome: A user can run `pastelsdd config profile` inside a workspace and choose whether to apply the changed global profile to that workspace now.

- [x] 3.1 Detect when `pastelsdd config profile` runs from inside an Pastelsdd workspace.
- [x] 3.2 After an actual profile or delivery change inside a workspace, prompt to apply changes to the current workspace now.
- [x] 3.3 When confirmed, run `pastelsdd workspace update` for the current workspace instead of repo-local `pastelsdd update`.
- [x] 3.4 When declined, report that global config changed and that `pastelsdd workspace update` applies it later.
- [x] 3.5 Preserve existing repo-local `pastelsdd config profile` apply behavior outside workspaces.
- [x] 3.6 Keep `pastelsdd config profile core` non-interactive, but print workspace-specific `pastelsdd workspace update` guidance when run inside a workspace.
- [x] 3.7 Warn on no-op config profile inside a workspace when workspace-local skills drift from the active global profile.
- [x] 3.8 Manually run `pastelsdd config profile` inside a workspace for confirm, decline, no-op, drift-warning, and `core` preset paths.
...
- [x] 4.8 Ensure creating a workspace change does not create repo-local Pastelsdd artifacts or edit linked repos.
...
- [x] 5.2 Enrich `pastelsdd status --change <id> --json` with planning home, change root, relevant artifact paths, affected areas, next steps, and action context.
- [x] 5.3 Enrich `pastelsdd instructions <artifact> --change <id> --json` with resolved artifact paths for repo-local and workspace-scoped changes.
...
- [x] 6.1 Update generated workflow skill templates to run `pastelsdd status --change <id> --json` before artifact work and trust returned planning context.
- [x] 6.2 Update generated workflow skill templates to run `pastelsdd instructions <artifact> --change <id> --json` before writing artifacts and use the resolved output path.
- [x] 6.3 Audit source workflow templates for hardcoded `pastelsdd/changes/<name>` assumptions and replace them with CLI-reported path guidance.
...
- [x] 7.4 Add tests that `pastelsdd config profile` inside a workspace can apply changes through `pastelsdd workspace update`.
- [x] 7.5 Add tests for stored workspace skill agent selection, omitted-`--tools` behavior, and profile drift reporting.
- [x] 7.6 Add tests that `pastelsdd update` from a workspace planning home redirects to `pastelsdd workspace update`.
...
- [x] 7.13 Run `pastelsdd validate workspace-change-planning --strict`.
...
- Generated workflow skills used CLI-reported paths and workspace guards rather than hardcoded `pastelsdd/changes/<name>` paths.
