...
The metadata file (`.pastelsdd.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .pastelsdd.yaml
 */
...
**Location:** `pastelsdd/changes/<name>/.pastelsdd.yaml`
...
- `pastelsdd.json` - YAML matches existing schema files
...
2. **`.pastelsdd.yaml` metadata** (persisted choice)
...
No migration needed:
- Existing changes without `.pastelsdd.yaml` continue to work (use default)
- New changes created with `pastelsdd new change --schema X` get metadata file

## Open Questions

- Should `pastelsdd new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
