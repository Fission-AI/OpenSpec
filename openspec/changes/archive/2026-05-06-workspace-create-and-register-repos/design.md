...
I can ask Pastelsdd what is broken and how to fix it.
```
...
The path may point at a full repo or a folder inside a large monorepo. It may point at a repo or folder that has not adopted repo-local Pastelsdd yet.
...
Path handling should behave like a folder picker. The user may type a relative or absolute path, but Pastelsdd should verify that it points to an existing folder, convert it to an absolute path relative to the command's current working directory when needed, and store that verified absolute path in local workspace state. Pastelsdd should not store the raw string the user typed.

Path conversion stays in the current runtime. Native Windows paths, WSL2 paths, and Unix paths should not be translated across runtimes. Where duplicate-path detection needs canonical comparisons, Pastelsdd may compare canonical existing paths internally, but it should store and display the verified absolute path for the current runtime.
...
Choose a different name:
  pastelsdd workspace link archived-api /archive/api

If you meant to change the existing link path:
  pastelsdd workspace relink api /archive/api
```
...
Show known Pastelsdd-managed workspaces from the local workspace registry.
...
workspaces:
  - name: platform
    location: /.../pastelsdd/workspaces/platform
    links:
      - name: api
        path: /repos/api
      - name: web
        path: /repos/web
  - name: checkout
    location: /.../pastelsdd/workspaces/checkout
    links:
      - name: app
...
```json
{
  "workspaces": [
    {
      "name": "platform",
      "root": "/.../pastelsdd/workspaces/platform",
      "links": [
        {
          "name": "api",
          "path": "/repos/api",
    ...
    {
      "name": "old-platform",
      "root": "/.../pastelsdd/workspaces/old-platform",
      "links": [],
      "status": [
        {
          "severity": "error",
...
- repos or folders without repo-local `pastelsdd/`

If the user passes a relative path, Pastelsdd should resolve it against the command's current working directory before writing local state.

If the path has repo-local Pastelsdd state, Pastelsdd can report the repo specs path in doctor output. If it does not, Pastelsdd should still allow workspace planning.
...
```json
{
  "workspace": {
    "name": "platform",
    "root": "/.../pastelsdd/workspaces/platform",
    "planning_path": "/.../pastelsdd/workspaces/platform/changes",
    "links": [
      {
        "name": "api",
        "path": "/repos/api",
        "repo_specs_path": "/repos/api/pastelsdd/specs",
        "status": []
      },
      {
        "name": "web",
        ...
        "status": [
          {
            ...
            "fix": "pastelsdd workspace relink web /path/to/web"
          }
        ]
      }
    ],
...
- otherwise explain that no workspaces exist and suggest `pastelsdd workspace setup`
...
```text
/.pastelsdd-workspace/local.yaml
```
...
- do not require repo-local Pastelsdd state to link a repo or folder
