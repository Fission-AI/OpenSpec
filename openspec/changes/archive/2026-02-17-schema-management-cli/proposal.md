...
Add a new `pastelsdd schema` command group with subcommands for creating, forking, validating, and inspecting schemas.

### Commands

1. **`pastelsdd schema init <name>`** - Interactive wizard to scaffold a new project schema
   ...
   - Optionally sets as project default in `pastelsdd/config.yaml`

2. **`pastelsdd schema fork <source> [name]`** - Copy an existing schema as a starting point
...
3. **`pastelsdd schema validate [name]`** - Validate schema structure and templates
...
4. **`pastelsdd schema which <name>`** - Show schema resolution path
