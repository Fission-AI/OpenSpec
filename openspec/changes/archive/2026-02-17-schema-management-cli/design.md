...
### 1. Command Structure: `pastelsdd schema <subcommand>`

Add a new command group following the existing pattern used by `pastelsdd config` and `pastelsdd completion`.
...
- Flat commands (`pastelsdd schema-init`, `pastelsdd schema-fork`): Rejected because it pollutes the top-level namespace and doesn't scale well.
- Extending existing commands (`pastelsdd init --schema`): Rejected because schema management is distinct from project initialization.
...
**Rationale:** Matches the UX established by `pastelsdd init` and `pastelsdd config reset`. Provides a guided experience while keeping the wizard lightweight.
...
The destination is always project-local: `pastelsdd/schemas/<name>/`
...
**Rationale:** Consistent with existing Pastelsdd commands and enables scripting/automation.
