...
**Workflow becomes:**
```
/pastel:new → /pastel:continue → /pastel:apply → archive
                                  │
                                  └── /pastel:sync (optional, anytime)
```

## Capabilities

### New Capabilities
- `specs-sync-skill`: Skill template for `/pastel:sync` command that reconciles main specs with delta specs
...
- **Skills**: New `pastelsdd-sync-specs` skill in `skill-templates.ts`
