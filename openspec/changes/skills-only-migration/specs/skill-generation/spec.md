...
### Requirement: Multi-editor skill generation
The system SHALL generate appropriate files for each selected editor using a unified transformation.

#### Scenario: Generate for Claude Code
- **WHEN** Claude Code is selected during generation
- **THEN** system creates .claude/skills/ and .claude/commands/opsx/

#### Scenario: Generate for Cursor
- **WHEN** Cursor is selected during generation
- **THEN** system creates appropriate Cursor rules files

#### Scenario: Generate for multiple editors
- **WHEN** multiple editors are selected
- **THEN** system generates files for each editor
- **AND** skill content is equivalent across editors (format may differ)
