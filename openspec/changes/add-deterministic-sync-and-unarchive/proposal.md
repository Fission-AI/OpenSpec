...
- `src/cli/index.ts` — register `sync [change]` (`--check`, `--fix`, `--json`, `--store`), `unarchive [change-name]` (`--keep-specs`/`--skip-specs` alias, `-y/--yes`, `--no-validate`, `--json`, `--store`), and `format [target]` (`--check`, `--fix`, `--json`), mirroring archive (326-343).
...
- **CI drift gate + pre-commit hook** — the CLI primitives (`sync --check`/`--fix`, `format --check`/`--fix`) are in scope; wiring a hook runner (none exists in the repo) or a CI job is staged (design Decision 4).
