...
- **Spec linter/formatter** — a "prettier-for-specs" formatter and richer organization lint (the discussion's "the framework's goal is how the spec looks and reads and is organized"). This PR makes *output* specs canonical and byte-stable; an authoring-side formatter is adjacent to `openspec-conventions`/`validate` and stays a separable follow-up (design Decision 13).
