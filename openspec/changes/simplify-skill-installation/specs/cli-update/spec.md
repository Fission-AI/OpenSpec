...
- **WHEN** user runs `pastelsdd update`
...
- **WHEN** user runs `pastelsdd update`
- **AND** a tool has generated Pastelsdd command files
- **AND** that tool has no Pastelsdd skill files (commands-only delivery)
...
- **THEN** the system SHALL display: "New in this version: /pastel:propose (combines new + ff). Try 'pastelsdd config profile core' for the streamlined 4-workflow experience."
...
- **WHEN** user runs `pastelsdd update` multiple times
...
- **WHEN** user runs `pastelsdd update` non-interactively (e.g., in CI)
...
- **THEN** the system SHALL display: "Detected new tool: Windsurf. Run 'pastelsdd init' to add it."
...
- **THEN** the system SHALL display one consolidated message listing all detected tools, for example: "Detected new tools: GitHub Copilot, Windsurf. Run 'pastelsdd init' to add them."
...
### Requirement: Update requires an Pastelsdd project
The update command SHALL only run inside an initialized Pastelsdd project.

#### Scenario: Update outside a project
- **WHEN** user runs `pastelsdd update`
- **AND** no `pastelsdd/` directory exists in the current working directory
- **THEN** the system SHALL display: "No Pastelsdd project found. Run 'pastelsdd init' to set up."
...
- **WHEN** user runs `pastelsdd update`
- **AND** project has workflows not in current profile (e.g., user switched from custom to core or deselected workflows via `pastelsdd config profile`)
