...
- **[Risk] Scope cleanup can touch user directories.** -> Revalidate canonical containment at mutation time, use explicit generated-name lookup, preserve global targets unless `--allow-global-cleanup` is present, require confirmation or `--force`, and never clean before replacement verification.
