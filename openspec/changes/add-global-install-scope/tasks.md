...
- [ ] 5.4 Validate all desired mutations and every authorized existing cleanup-only target in a complete preflight, re-resolve and canonically revalidate containment immediately before each write, rename, marker update, or removal, use atomic same-directory replacement where supported, then build a symmetric project/global transition preview with exact destination and cleanup paths
...
- [ ] 5.8 Verify all desired new-target artifacts before confirmed non-effective-scope cleanup, and add unit tests for both transition directions, confirmation accept/decline, force without global authority, explicit global cleanup authorization, non-interactive requirements, invalid-config blocking, split scopes, all three surface classifications, override preservation, mutation-time symlink replacement, partial writes, cleanup failure, and ordinary-update repeat-run recovery; document the one-mutating-invocation model and simultaneous same-global-target mutation as unsupported rather than adding a cross-process lock, manifest, or downgrade rule
