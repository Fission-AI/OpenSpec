...
- **Duplication instead of reuse.** `propose` repeats ~100 lines of artifact-loop logic already present in `ff-change`. Every skill carries a second, near-identical `CommandTemplate` (~98% byte-identical for most; ~90% for `propose`), so each instruction is maintained in two places and drifts.
- **Unbounded always-on context.** `onboard` (564 lines), `bulk-archive-change` (250), and `verify-change` (173) load deep reference material into context every time the skill is read, even when only the core procedure is needed.
