...
- `cli-artifact-workflow`: `openspec status --json` includes per-artifact dependency edges (`requires`, `dependents`), a content `digest`, and a deterministic drift signal (current upstream digest vs. recorded baseline, or `unknown` when none); a new `--impact <artifact>` selector returns the downstream revisit set in build order (with paths and digests), and `--record` writes the digest baseline — so skills consume deterministic graph structure as data instead of hardcoding artifact names.
