...
This document proposes a lightweight roadmap for workspace, monorepo, and multi-repo support in Pastelsdd.
...
Pastelsdd needs a better way to organize:
...
References help agents and humans navigate related specs without requiring Pastelsdd to build a dependency graph system on day one.
...
1. Nested spec paths within one `pastelsdd/` root
...
If Pastelsdd uses local path caches or machine-specific mappings, they should be:
