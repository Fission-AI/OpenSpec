...
pastelsdd workspace open --initiative billing-launch
pastelsdd workspace open team-a-billing --initiative platform/billing-launch
```
...
- If no exact match exists, do not silently open the closest match. Show a small
  list of likely matches when available, plus a hint to run `pastelsdd
  initiative list`.
...
Store each private workspace view under the user's Pastelsdd global data
...
Item 10. The other files are generated runtime support owned by Pastelsdd. They
...
- Generated files are Pastelsdd-owned and may be overwritten each time.
...
for tools that need structured facts after Pastelsdd has prepared the workspace
...
`.pastelsdd-workspace/workspace.yaml`, `.pastelsdd-workspace/local.yaml`, and a
