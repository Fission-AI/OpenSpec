...
Final decision: do not implement a standalone `pastelsdd initiative resolve`
...
Do not ship `pastelsdd initiative resolve <id>` as a user-facing command in this
...
If a user only has a context store clone, Pastelsdd can already resolve the
canonical initiative with:

```bash
pastelsdd initiative show billing-launch --json
```
...
- Pastelsdd should not infer repo ownership, scan arbitrary repos, clone repos,
...
- Use `pastelsdd initiative show <id> --json` to locate canonical shared context.
- Use workspace commands to set up, link, relink, list, open, update, and doctor
  local views.
- Use repo-local `pastelsdd new change ... --initiative ...` and
  `pastelsdd set change ... --initiative ...` to create durable links from repo
  work to initiative context.
- Use `pastelsdd status --change <id> --json` inside the owning repo to inspect
...
Decision: do not add `pastelsdd initiative resolve`, now or later. Initiative
