...
- Start from `pastelsdd/initiatives/context-store-and-initiatives/direction.md`
  for product authority.
- Treat `pastelsdd/changes/workspace-reimplementation-roadmap/HISTORICAL_DIRECTION.md` and
  `pastelsdd/changes/workspace-reimplementation-roadmap/` as historical reference
...
- Pastelsdd can create and manipulate files inside a local context store without
...
Decision: do not add `pastelsdd initiative resolve`, now or later.
...
- `pastelsdd initiative resolve <id>`
...
```bash
pastelsdd initiative next billing-launch --json
```
...
```json
{
  ...
  "suggested_command": "pastelsdd new change add-billing-api --initiative billing-launch"
}
```
...
- Use `pastelsdd workspace open --initiative <initiative>`.
- Support `<store>/<initiative>` and `<initiative> --store <store>`.
- Support `pastelsdd workspace open <workspace-name> --initiative <initiative>`
...
- On no exact match, show likely matches when available and suggest `pastelsdd
...
Pastelsdd can upgrade it into a coordinated initiative and carry the current
...
Pastelsdd changes only after they are bound to a target repo or spec root,
...
- Audit commands, templates, validation, archive, apply, completion, and docs
  for repo-local `pastelsdd/changes/` assumptions.
