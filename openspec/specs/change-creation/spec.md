...
## Purpose
Provide programmatic utilities for creating and validating ClearSpec change directories.
## Requirements
...
- **WHEN** the user creates a new change
- **THEN** ClearSpec SHALL resolve the current workspace as the planning home
...
- **THEN** ClearSpec SHALL preserve repo-local change creation behavior for that location
...
- **GIVEN** the command runs outside a ClearSpec workspace
- **WHEN** the user creates a new change in a repo-local ClearSpec project
- **THEN** ClearSpec SHALL continue to create the change under `openspec/changes/`
...
- **THEN** ClearSpec SHALL reject those invalid affected areas
...
- **THEN** ClearSpec SHALL create the workspace change
