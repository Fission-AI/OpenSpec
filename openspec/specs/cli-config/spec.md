...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global ClearSpec configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `clearspec config --help`
...
- **WHEN** user executes `clearspec config path`
...
- **WHEN** user executes `clearspec config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `clearspec config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `clearspec config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `clearspec config get <key>` where the value is an object
...
- **WHEN** user executes `clearspec config set <key> <value>`
...
- **WHEN** user executes `clearspec config set <key> true` or `clearspec config set <key> false`
...
- **WHEN** user executes `clearspec config set <key> <value> --string`
...
- **WHEN** user executes `clearspec config set featureFlags.newFlag true`
...
- **WHEN** user executes `clearspec config unset <key>`
...
- **WHEN** user executes `clearspec config reset --all`
...
- **WHEN** user executes `clearspec config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `clearspec config reset` without `--all`
...
- **WHEN** user executes `clearspec config edit`
...
The `clearspec config profile` command SHALL provide an action-first interactive flow that allows users to modify delivery and workflow settings independently.

#### Scenario: Current profile summary appears first

- **WHEN** user runs `clearspec config profile` in an interactive terminal
...
- **WHEN** user runs `clearspec config profile` interactively
...
- **WHEN** delivery selection is shown in `clearspec config profile`
...
- **WHEN** `clearspec config profile` exits with `No config changes.` inside a ClearSpec project
...
- **AND** include guidance to run `clearspec update` to sync project files

#### Scenario: Apply prompt is gated on actual changes

- **WHEN** config values were changed and saved
- **AND** current directory is a ClearSpec project
- **THEN** prompt `Apply changes to this project now?`
- **AND** if confirmed, run `clearspec update` for the current project
...
- **WHEN** user executes `clearspec config set someFutureKey 123`
...
- **WHEN** user executes `clearspec config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `clearspec config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `clearspec config --scope project <subcommand>`
...
The `clearspec config profile` command SHALL remain global while offering an explicit workspace apply path when run from inside a ClearSpec workspace.

#### Scenario: Config profile run inside a workspace
- **GIVEN** the command runs from inside a ClearSpec workspace
- **WHEN** the user changes profile or delivery settings with interactive `clearspec config profile`
- **THEN** ClearSpec SHALL save the global config changes
- **AND** it SHALL prompt: `Apply changes to this workspace now?`

#### Scenario: User confirms workspace apply
- **GIVEN** `clearspec config profile` changed global profile or delivery settings inside a workspace
- **WHEN** the user confirms the workspace apply prompt
- **THEN** ClearSpec SHALL run `clearspec workspace update` for the current workspace
- **AND** it SHALL not run repo-local `clearspec update` unless the current planning home is repo-local

#### Scenario: User declines workspace apply
- **GIVEN** `clearspec config profile` changed global profile or delivery settings inside a workspace
- **WHEN** the user declines the workspace apply prompt
- **THEN** ClearSpec SHALL explain that global config was updated
- **AND** it SHALL tell the user to run `clearspec workspace update` later to apply the profile to workspace-local skills
- **AND** it SHALL not modify workspace skill files

#### Scenario: No-op inside workspace
- **GIVEN** the command runs from inside a ClearSpec workspace
- **WHEN** `clearspec config profile` exits with no effective config changes
- **THEN** ClearSpec SHALL not prompt to apply changes
- **AND** it SHALL warn if workspace-local skills are out of sync with the current global profile
- **AND** the warning SHALL suggest `clearspec workspace update`

#### Scenario: Core preset shortcut inside a workspace
- **GIVEN** the command runs from inside a ClearSpec workspace
- **WHEN** the user runs `clearspec config profile core`
- **THEN** ClearSpec SHALL save the global config change without prompting to apply immediately
- **AND** it SHALL tell the user to run `clearspec workspace update` to apply the profile to workspace-local skills

#### Scenario: Core preset shortcut inside a repo project
- **GIVEN** the command runs from inside a repo-local ClearSpec project
- **WHEN** the user runs `clearspec config profile core`
- **THEN** ClearSpec SHALL preserve existing repo-local shortcut behavior
- **AND** it SHALL tell the user to run `clearspec update` to apply the profile to project files

#### Scenario: Workspace planning home wins over linked repo project
- **GIVEN** the command runs in a path under a workspace planning home where a repo-local ClearSpec project could also be detected
- **WHEN** ClearSpec decides which apply prompt to show
- **THEN** the nearest current planning home SHALL determine whether to offer `clearspec workspace update` or repo-local `clearspec update`
- **AND** ClearSpec SHALL not apply profile changes to a linked repo when the current planning home is the workspace

#### Scenario: Linked repo keeps repo-local profile behavior
- **GIVEN** a repo-local ClearSpec project is registered as a workspace link
- **AND** the command runs from inside that linked repo rather than from the workspace planning home
- **WHEN** ClearSpec decides which apply prompt or guidance to show
- **THEN** ClearSpec SHALL preserve repo-local `clearspec update` behavior for that repo
- **AND** it SHALL not offer `clearspec workspace update` unless the workspace is explicitly selected
