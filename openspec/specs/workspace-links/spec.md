...
and JSON-output behavior for managing ClearSpec workspaces across repos and
folders.
## Requirements
### Requirement: Guided Workspace Setup
ClearSpec SHALL provide a guided setup flow for users starting workspace planning.

#### Scenario: Creating a workspace through setup
- **WHEN** a user runs `clearspec workspace setup`
- **THEN** ClearSpec SHALL guide the user through creating a ClearSpec workspace
...
- **THEN** ClearSpec SHALL ask for the workspace name before asking for repos or folders
...
- **THEN** ClearSpec SHALL explain that workspace names must be kebab-case
...
- **THEN** ClearSpec SHALL infer the link name from the folder basename
...
- **THEN** ClearSpec SHALL show the conflicting link name and the existing path for that link
...
- **THEN** ClearSpec SHALL let the user add another repo or folder with a simple repeated prompt
...
- **THEN** ClearSpec SHALL verify that the path resolves to an existing folder
...
- **THEN** ClearSpec SHALL treat the full value as the path
...
- **WHEN** `clearspec workspace setup --no-interactive` receives a workspace name and at least one valid link
- **THEN** ClearSpec SHALL create the workspace without prompts
- **AND** it SHALL support repeated `--link` values

#### Scenario: Non-interactive setup duplicate link names
- **WHEN** `clearspec workspace setup --no-interactive` receives two links with the same inferred or explicit name
- **THEN** ClearSpec SHALL fail with a clear duplicate link-name error
...
- **WHEN** `clearspec workspace setup --no-interactive` is missing a workspace name or link
- **THEN** ClearSpec SHALL fail with a clear message
- **AND** it SHALL explain which flags are required

#### Scenario: Finishing setup
- **WHEN** setup finishes
- **THEN** ClearSpec SHALL show the workspace location, planning path, and linked repos or folders
...
- **THEN** ClearSpec SHALL record it in the local workspace registry
...
- **THEN** ClearSpec SHALL explain that the workspace already exists
- **AND** it SHALL not overwrite the existing workspace

### Requirement: Workspace Discovery
ClearSpec SHALL let users see the ClearSpec-managed workspaces available on the current machine.

#### Scenario: Listing workspaces
- **WHEN** a user runs `clearspec workspace list`
- **THEN** ClearSpec SHALL list known managed workspaces
...
- **WHEN** a user runs `clearspec workspace ls`
- **THEN** ClearSpec SHALL behave the same as `clearspec workspace list`

#### Scenario: Listing when no workspaces exist
- **WHEN** a user runs `clearspec workspace list`
- **AND** no managed workspaces exist
- **THEN** ClearSpec SHALL say that no workspaces were found
...
- **THEN** ClearSpec SHALL use the current workspace
...
- **THEN** ClearSpec SHALL record the workspace name and location in the local workspace registry
...
- **THEN** ClearSpec SHALL report the non-fatal registry warning
...
- **THEN** ClearSpec SHALL show a workspace picker
...
- **THEN** ClearSpec SHALL fail without showing a picker
...
- **THEN** ClearSpec SHALL fail without showing a picker regardless of interactive mode
- **AND** it SHALL print `No known ClearSpec workspaces. Run 'clearspec workspace setup' first.`
...
ClearSpec SHALL let users link existing repos or folders to a workspace before creating a change.

#### Scenario: Linking with an inferred name
- **WHEN** a user runs `clearspec workspace link <path>`
- **THEN** ClearSpec SHALL infer the link name from the folder basename
- **AND** it SHALL store the verified absolute local path as machine-local state

#### Scenario: Linking with an explicit name
- **WHEN** a user runs `clearspec workspace link <name> <path>`
- **THEN** ClearSpec SHALL use the explicit link name for planning
- **AND** it SHALL store the verified absolute local path as machine-local state
...
- **AND** ClearSpec SHALL reject missing paths with a clear message

#### Scenario: Resolving linked paths before storage
- **WHEN** a user links a repo or folder path
- **THEN** ClearSpec SHALL store the verified absolute path for the current runtime
- **AND** relative inputs SHALL be resolved against the command's current working directory
- **AND** ClearSpec SHALL not translate paths between native Windows, WSL2, and Unix runtimes

#### Scenario: Linking a monorepo folder
- **WHEN** a user links a package, service, app, or directory inside a monorepo
- **THEN** ClearSpec SHALL store it as a workspace link
- **AND** it SHALL not require that folder to have its own repo-local `openspec/` directory

#### Scenario: Linking without repo-local ClearSpec
- **WHEN** a user links a path that does not contain repo-local ClearSpec state
- **THEN** ClearSpec SHALL keep that repo or folder available for workspace planning
- **AND** it SHALL not treat missing repo-local ClearSpec state as a link failure

#### Scenario: Link records only
- **WHEN** a user links a repo or folder
- **THEN** ClearSpec SHALL record workspace state and local path state
...
- **WHEN** a user runs `clearspec workspace link`
- **THEN** ClearSpec SHALL fail with status code `workspace_local_state_invalid`
...
- **THEN** ClearSpec SHALL explain that the link name is already in use by another link
...
ClearSpec SHALL let users update existing link paths without recreating the workspace.

#### Scenario: Updating a local path
- **GIVEN** a workspace has a link
- **WHEN** a user runs `clearspec workspace relink <name> <path>`
- **THEN** ClearSpec SHALL keep the stable link name
...
- **GIVEN** the workspace machine-local state file exists but cannot be parsed or validated
- **WHEN** a user runs `clearspec workspace relink`
- **THEN** ClearSpec SHALL fail with status code `workspace_local_state_invalid`
...
- **THEN** ClearSpec SHALL explain that the link name is unknown
...
- **THEN** ClearSpec SHALL not ask for owner or handoff metadata
- **AND** link maintenance SHALL focus on names and local paths

### Requirement: Workspace Health Check
ClearSpec SHALL explain what the current machine can resolve for a workspace.

#### Scenario: Doctor checks one selected workspace
- **WHEN** a user runs `clearspec workspace doctor`
- **THEN** ClearSpec SHALL inspect one selected workspace
- **AND** it SHALL not scan every known workspace in the local registry by default

#### Scenario: Doctor infers the current workspace
- **GIVEN** the command runs from a workspace folder or subdirectory
- **WHEN** the user runs `clearspec workspace doctor` without `--workspace <name>`
- **THEN** ClearSpec SHALL inspect the current workspace

#### Scenario: Checking a healthy workspace
- **WHEN** a user runs `clearspec workspace doctor`
- **THEN** ClearSpec SHALL show the workspace location and workspace planning path
...
- **WHEN** a user runs `clearspec workspace doctor`
- **THEN** ClearSpec SHALL report a selected-workspace status error
...
- **THEN** ClearSpec SHALL report status code `workspace_local_state_invalid`
...
ClearSpec SHALL provide JSON output for direct workspace setup commands.

#### Scenario: Requesting JSON output
- **WHEN** a user passes `--json` to direct workspace setup commands
- **THEN** ClearSpec SHALL print machine-readable output
...
- **WHEN** a user runs `clearspec workspace setup --json` without `--no-interactive`
- **THEN** ClearSpec SHALL fail clearly
...
- **THEN** ClearSpec SHALL avoid interactive prompts
...
ClearSpec SHALL let users install ClearSpec agent skills into a workspace during workspace setup.

#### Scenario: Prompting for workspace agent skills
- **WHEN** interactive workspace setup reaches agent skill installation
- **THEN** ClearSpec SHALL ask which agents should get ClearSpec skills in this workspace
- **AND** the prompt SHALL use agent-skill language rather than "AI tools" language

#### Scenario: Preselecting the preferred opener
- **GIVEN** the user selected a preferred opener that supports ClearSpec skill generation
- **WHEN** interactive workspace setup asks which agents should get skills
- **THEN** ClearSpec SHALL preselect the matching agent
...
- **THEN** ClearSpec SHALL generate or refresh ClearSpec skill files under the workspace root for each selected agent
...
- **THEN** ClearSpec SHALL install workspace-local skills for the workflows selected by that profile
...
- **WHEN** workspace setup installs agent skills
- **THEN** ClearSpec SHALL generate skill files only
...
- **WHEN** workspace setup installs agent skills
- **THEN** ClearSpec SHALL still generate workspace-local skills only
...
- **WHEN** workspace setup installs agent skills
- **THEN** ClearSpec SHALL leave linked repos and folders unchanged
...
- **THEN** ClearSpec SHALL use the selected tool set for workspace agent skill installation
...
- **THEN** ClearSpec SHALL create the workspace without installing agent skills
- **AND** it SHALL report that no workspace skills were installed
- **AND** it SHALL tell the user to run `clearspec workspace update --tools <ids>` to install skills later
...
- **THEN** ClearSpec SHALL include generated, refreshed, skipped, or failed skill installation results in machine-readable output

### Requirement: Workspace update manages agent skills
ClearSpec SHALL provide a workspace update flow for refreshing agent skills after setup.

#### Scenario: Updating the current workspace
- **GIVEN** the command runs from inside a ClearSpec workspace
- **WHEN** the user runs `clearspec workspace update`
- **THEN** ClearSpec SHALL update that current workspace

#### Scenario: Updating a named workspace
- **GIVEN** a workspace named `platform` is known locally
- **WHEN** the user runs `clearspec workspace update platform`
- **THEN** ClearSpec SHALL update the `platform` workspace

#### Scenario: Updating a workspace selected by flag
- **GIVEN** a workspace named `platform` is known locally
- **WHEN** the user runs `clearspec workspace update --workspace platform`
- **THEN** ClearSpec SHALL update the `platform` workspace

#### Scenario: Updating selected workspace skills
- **WHEN** workspace update completes with selected agents
- **THEN** ClearSpec SHALL refresh ClearSpec skills for selected agents
- **AND** it SHALL add skills for newly selected agents
- **AND** it SHALL remove ClearSpec-managed workflow skill directories for agents that are no longer selected
...
- **THEN** ClearSpec SHALL treat it as ClearSpec-managed only when the directory name matches a known generated workflow skill directory and its `SKILL.md` contains ClearSpec generated metadata
- **AND** generated metadata SHALL include the `generatedBy` marker written by ClearSpec skill generation
- **AND** ClearSpec SHALL not remove directories that are missing the generated metadata, even when their names match known workflow skill directory names

#### Scenario: Updating profile-selected workflows
- **GIVEN** global config resolves to a workflow profile
- **WHEN** workspace update refreshes workspace-local skills
- **THEN** ClearSpec SHALL sync the workspace-local skill workflow set to the workflows selected by that profile
- **AND** deselected workflow skill directories SHALL be removed only when they are known ClearSpec-managed workflow skill directories
...
- **GIVEN** global config delivery is `commands` or `both`
- **WHEN** workspace update refreshes workspace-local skills
- **THEN** ClearSpec SHALL still update workspace-local skills only
...
- **THEN** ClearSpec SHALL remove only known ClearSpec-managed workflow skill directories
...
- **THEN** ClearSpec SHALL replace the stored workspace-local selected agent list with that selection
...
- **THEN** ClearSpec SHALL update workspace agent skills using that selected tool set
...
- **THEN** ClearSpec SHALL refresh the stored selected agents using the active global profile
- **AND** it SHALL avoid prompting for agent selection

#### Scenario: Non-interactive update without stored selection
- **GIVEN** no workspace-local selected agents are stored
- **WHEN** non-interactive workspace update omits `--tools`
- **THEN** ClearSpec SHALL complete without installing agent skills
...
- **WHEN** ClearSpec reports workspace skill state
- **THEN** it SHALL report that workspace-local skills are out of sync with the global profile
- **AND** it SHALL suggest `clearspec workspace update`
...
- **THEN** ClearSpec SHALL report which agents were refreshed, added, removed, skipped, or failed

#### Scenario: Reporting workspace update results in JSON output
- **WHEN** workspace update runs with JSON output enabled
- **THEN** ClearSpec SHALL include refreshed, added, removed, skipped, or failed skill results in machine-readable output

### Requirement: Workspace skill update surface is documented
ClearSpec SHALL expose workspace skill setup/update behavior in user-facing command surfaces.

#### Scenario: Workspace update appears in help
- **WHEN** a user runs `clearspec workspace --help`
- **THEN** ClearSpec SHALL list `workspace update`
- **AND** it SHALL describe it as refreshing workspace-local agent skills

#### Scenario: Workspace update options appear in help
- **WHEN** a user runs `clearspec workspace update --help`
- **THEN** ClearSpec SHALL document workspace selection options
