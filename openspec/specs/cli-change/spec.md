...
## Purpose
Define `clearspec change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `clearspec change show update-error --json`
...
- **WHEN** executing `clearspec change list`
...
- **WHEN** executing `clearspec change show update-error --requirements-only`
...
- **WHEN** executing `clearspec change validate update-error`
...
- **WHEN** executing `clearspec list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'clearspec list' is deprecated. Use 'clearspec change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `clearspec list --all`
...
- **WHEN** executing `clearspec change show` without arguments
...
- **WHEN** executing `clearspec change show` without a change name
...
- **WHEN** executing `clearspec change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `clearspec change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
