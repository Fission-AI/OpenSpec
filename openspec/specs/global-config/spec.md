...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/clearspec`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/clearspec` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\clearspec`
