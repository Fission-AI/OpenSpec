...
- **AND** treat a capability whose last requirement the sync removed as verified when its main spec was moved into the change's `retired-specs/` rather than left empty, and a spec the sync deliberately kept and reported as verified too
