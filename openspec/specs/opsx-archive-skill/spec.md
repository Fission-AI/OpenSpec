...
Define the expected behavior for the `/clsx:archive` skill, including readiness checks, spec sync prompting, archive execution, and user-facing output.

## Requirements

### Requirement: OPSX Archive Skill

The system SHALL provide an `/clsx:archive` skill that archives completed changes in the experimental workflow.

#### Scenario: Archive a change with all artifacts complete

- **WHEN** agent executes `/clsx:archive` with a change name
...
- **WHEN** agent executes `/clsx:archive` without specifying a change
...
- **AND** if user confirms, execute `/clsx:sync` logic
...
- **THEN** display summary:
  - Specs synced (from `/clsx:sync` output)
