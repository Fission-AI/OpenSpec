...
## Purpose
Define top-level `clearspec show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `clearspec show` without arguments
...
- **WHEN** executing `clearspec show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `clearspec show <item>` or `clearspec change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `clearspec show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `clearspec change show`/`clearspec spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `clearspec show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `clearspec show --type spec <item>`
...
- **WHEN** executing `clearspec show <item> --json`
...
- **WHEN** showing a change with `clearspec show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `clearspec show <spec-id> --requirements`
