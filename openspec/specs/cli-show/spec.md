...
## Purpose
Define top-level `pastelsdd show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `pastelsdd show` without arguments
...
- **WHEN** executing `pastelsdd show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `pastelsdd show <item>` or `pastelsdd change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `pastelsdd show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `pastelsdd change show`/`pastelsdd spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `pastelsdd show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `pastelsdd show --type spec <item>`
...
- **WHEN** executing `pastelsdd show <item> --json`
...
- **WHEN** showing a change with `pastelsdd show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `pastelsdd show <spec-id> --requirements`
