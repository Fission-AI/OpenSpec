...
| Surface | Behavior |
| --- | --- |
| Install scripts | None. The package ships no `postinstall` or other lifecycle script, so `npm install` runs no code from OpenSpec. Shell completions are opt-in via `openspec completion install`; the CLI prints a one-line tip about them on its first run. |
