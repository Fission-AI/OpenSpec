...
const METADATA_FILENAME = '.clearspec.yaml';
...
/**
 * Writes change metadata to .clearspec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .clearspec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .clearspec.yaml metadata (if exists)
 * 3. Schema from clearspec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/clearspec/changes/change-name)
