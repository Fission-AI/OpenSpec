...
const METADATA_FILENAME = '.lightspec.yaml';
...
/**
 * Writes change metadata to .lightspec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .lightspec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .lightspec.yaml metadata (if exists)
 * 3. Schema from lightspec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/lightspec/changes/change-name)
