...
export async function generateApplyInstructions(
  ...
  if (missingArtifacts.length > 0) {
    state = 'blocked';
    instruction = `Cannot apply this change yet. Missing artifacts: ${missingArtifacts.join(', ')}.\nUse the pastelsdd-continue-change skill to create the missing artifacts first.`;
  } else if (tracksFile && !tracksFileExists) {
    // Tracking file configured but doesn't exist yet
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file is missing and must be created.\nUse pastelsdd-continue-change to generate the tracking file.`;
  } else if (tracksFile && tracksFileExists && total === 0) {
    // Tracking file exists but contains no tasks
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file exists but contains no tasks.\nAdd tasks to ${tracksFilename} or regenerate it with pastelsdd-continue-change.`;
...
export function printApplyInstructionsText(instructions: ApplyInstructions): void {
  ...
  if (state === 'blocked' && missingArtifacts) {
    ...
    console.log('Use the pastelsdd-continue-change skill to create these first.');
    console.log();
  }
