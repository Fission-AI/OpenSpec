...
async function toolSelectionWizard(config: ToolWizardConfig): Promise<string[]> {
  const {
    createPrompt,
    isBackspaceKey,
    isDownKey,
  ...
  const prompt = createPrompt<string[], ToolWizardConfig>(
    (config, done) => {
      const totalSteps = 3;
...
export class InitCommand {
  ...
  constructor(options: InitCommandOptions = {}) {
    this.prompt = options.prompt ?? toolSelectionWizard;
