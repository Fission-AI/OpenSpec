...
async function maybeWarnConfigDrift(
  ...
  const workspaceContext = scope === 'user' ? await resolveWorkspaceConfigProfileContext() : null;
  if (workspaceContext) {
    let localState = null;
    try {
  ...
  maybeWarnProjectConfigDrift(process.cwd(), state, colorize, scope);
}
    ...
    .action(async (preset?: string) => {
      ...
      try {
        ...
        if (action === 'keep') {
          ...
          await maybeWarnConfigDrift(currentState, chalk.yellow, scope);
          return;
        }
        ...
        if (!diff.hasChanges) {
          console.log('No config changes.');
          await maybeWarnConfigDrift(nextState, chalk.yellow, scope);
          return;
        }
