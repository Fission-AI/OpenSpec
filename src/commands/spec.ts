...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'lightspec' as const };
...
export class SpecCommand {
  private SPECS_DIR = 'lightspec/specs';
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'lightspec' as const },
      };
