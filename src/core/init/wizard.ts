...
const toolSelectionWizardPrompt = createPrompt<string[], ToolWizardConfig>(
  (config, done) => {
    const totalSteps = 3;
 ...
 * This wrapper function provides an explicit return type to avoid TypeScript's
 * inability to express the internal inquirer Prompt type in public declarations.
 */
export function toolSelectionWizard(config: ToolWizardConfig): Promise<string[]> {
  return toolSelectionWizardPrompt(config);
}
