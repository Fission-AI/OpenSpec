...
export async function writeUpdatedSpec(
  ...
  const specName = path.basename(path.dirname(update.target));
  console.log(`Applying changes to pastelsdd/specs/${specName}/spec.md:`);
...
export async function applySpecs(
  ...
  const changeDir = path.join(projectRoot, 'pastelsdd', 'changes', changeName);
  const mainSpecsDir = path.join(projectRoot, 'pastelsdd', 'specs');
  ...
  for (const p of prepared) {
    ...
    if (!options.dryRun) {
      ...
      if (!options.silent) {
        console.log(`Applying changes to pastelsdd/specs/${capability}/spec.md:`);
      ...
      console.log(`Would apply changes to pastelsdd/specs/${capability}/spec.md:`);
      if (p.counts.added) console.log(`  + ${p.counts.added} added`);
      if (p.counts.modified) console.log(`  ~ ${p.counts.modified} modified`);
      if (p.counts.removed) console.log(`  - ${p.counts.removed} removed`);
      if (p.counts.renamed) console.log(`  → ${p.counts.renamed} renamed`);
