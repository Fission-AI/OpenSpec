 ...
 * File path: .pi/prompts/clsx-<id>.md
 * Frontmatter: description
 *
 * Pi uses the filename (minus .md) as the slash command name, so
 * clsx-propose.md → /clsx-propose. Command references in the body
 * are transformed from /clsx: to /clsx- for consistency.
 */
export const piAdapter: ToolCommandAdapter = {
  toolId: 'pi',

  getFilePath(commandId: string): string {
    return path.join('.pi', 'prompts', `clsx-${commandId}.md`);
  },

  formatFile(content: CommandContent): string {
    // Transform /clsx: references to /clsx- and inject $@ for template args
