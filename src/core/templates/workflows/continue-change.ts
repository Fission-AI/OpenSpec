...
export function getContinueChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'pastelsdd-continue-change',
    description: 'Continue working on an Pastelsdd change by creating the next artifact. Use when the user wants to progress their change, create the next artifact, or continue their workflow.',
...
1. **If no change name provided, prompt for selection**
   ...
   Run \`pastelsdd list --json\` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.
