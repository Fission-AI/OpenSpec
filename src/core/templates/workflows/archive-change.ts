...
export function getPastelArchiveCommandTemplate(): CommandTemplate {
  return {
    name: 'Pastel: Archive',
...
**Input**: Optionally specify a change name (e.g., \`/pstl:archive add-auth\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.

**Steps**

1. **If no change name provided, prompt for selection**

   Run \`pastelsdd list --json\` to get available changes. Use the **AskUserQuestion tool** to let the user select.
...
2. **Check artifact completion status**
   ...
   Run \`pastelsdd status --change "<name>" --json\` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **If delta specs exist:**
   - Compare each delta spec with its corresponding main spec at \`pastelsdd/specs/<capability>/spec.md\`
   ...
   **Prompt options:**
   ...
   If user chooses sync, use Task tool (subagent_type: "general-purpose", prompt: "Use Skill tool to invoke pastelsdd-sync-specs for change '<name>'. Delta spec analysis: <include the analyzed delta spec summary>"). Proceed to archive regardless of choice.
   ...
   **If \`lists.done\` is configured:**
   ...
   - **No card found:** create one in \`lists.done\`:
     ...
     mcp__claude_ai_Trello_Custom__create_card
       ...
       desc: "Arquivada via /pstl:archive"
     \`\`\`
   ...
   mcp__claude_ai_Trello_Custom__add_comment
     card_id: "<cardId>"
     text: |
       Change arquivada via /pstl:archive
...
- Use artifact graph (pastelsdd status --json) for completion checking
- Don't block archive on warnings — just inform and confirm
- Preserve .pastelsdd.yaml when moving to archive (it moves with the directory)
- Show clear summary of what happened
- If sync is requested, use pastelsdd-sync-specs approach (agent-driven)
