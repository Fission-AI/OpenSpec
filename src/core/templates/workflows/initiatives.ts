...
**Advance the workflow.** The transition rule is one sentence: read everything upstream of where you stand (evergreen files, lower-numbered stages), then produce what your stage owes the next one. Never re-interview for what an upstream artifact already answers — cite it; when upstream is silent on something you need, say so and ask once. This one rule is why no persona needs its own skill: whoever owns a stage — analyst, PM, architect, designer, engineer, or a hat-switching solo dev — is making the same move from a different position, and decomposing the last stage into changes is just the final instance of it.
