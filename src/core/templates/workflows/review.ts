 ...
 *
 * The review workflow: an iterative review-critique-implement loop that
 * audits a change implementation with read-only reviewer and critic roles,
 * dispatches surgical fixers for confirmed issues, and stops only when the
 * four-condition consensus gate passes.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';
import { STORE_SELECTION_GUIDANCE } from './store-selection.js';

const REVIEW_SKILL_INSTRUCTIONS = `Run an iterative review–critique–implement loop on an OpenSpec change: spawn subagents from the self-contained role briefs below, apply fixes where issues are found, and repeat until the consensus check passes.
...
**Input**: Optionally specify a change name (e.g., \`/opsx:review add-auth\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Reviewing change: <name>" and how to override (e.g., \`/opsx:review <other>\`).
...
**Input**: Optionally specify a change name after \`/opsx:review\` (e.g., \`/opsx:review add-auth\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
export function getOpsxReviewCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: Review',
    description: 'Run an iterative review-critique-implement loop on a change before archiving',
    category: 'Workflow',
    tags: ['workflow', 'review', 'experimental'],
    content: REVIEW_COMMAND_CONTENT,
  };
}
