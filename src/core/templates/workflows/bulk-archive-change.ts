...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync specs** if delta specs exist:
      - Run the \`/opsx:sync\` workflow inline (agent-driven intelligent merge, or merge delta specs directly into main specs at \`<planningHome.root>/openspec/specs/<capability>/spec.md\` if sync workflow is not installed) for each change, passing the delta spec analysis, and wait for it to finish.
      - For conflicts, apply in resolved order.
      - Do not delegate to a background task — step 8c would move \`changeRoot\` out from under a sync that is still reading it.
