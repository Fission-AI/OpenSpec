...
export function getBulkArchiveChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'clearspec-bulk-archive-change',
...
1. **Get active changes**
   ...
   Run \`clearspec list --json\` to get all active changes.
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run \`clearspec status --change "<name>" --json\`
...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync specs** if delta specs exist:
      - Use the clearspec-sync-specs approach (agent-driven intelligent merge)
...
- Preserve .clearspec.yaml when moving to archive
...
export function getClsxBulkArchiveCommandTemplate(): CommandTemplate {
  return {
    name: 'CLSX: Bulk Archive',
