...
/**
 * Template for lightspec-explore skill
...
**IMPORTANT: Explore mode is for thinking, not implementing.** You may read files, search code, and investigate the codebase, but you must NEVER write code or implement features. If the user asks you to implement something, remind them to exit explore mode first (e.g., start a change with \`/opsx:new\` or \`/opsx:ff\`). You MAY create LightSpec artifacts (proposals, designs, specs) if the user asks—that's capturing thinking, not implementing.
...
You have full context of the LightSpec system. Use it naturally, don't force it.
...
1. **Read existing artifacts for context**
   - \`lightspec/changes/<name>/proposal.md\`
   - \`lightspec/changes/<name>/design.md\`
   - \`lightspec/changes/<name>/tasks.md\`
   - etc.
...
- **Don't implement** - Never write code or implement features. Creating LightSpec artifacts is fine, writing application code is not.
...
/**
 * Template for lightspec-new-change skill
...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run \`lightspec schemas --json\` and let them choose

   **Otherwise**: Omit \`--schema\` to use the default.

3. **Create the change directory**
   \`\`\`bash
   lightspec new change "<name>"
   \`\`\`
   Add \`--schema <name>\` only if the user requested a specific workflow.
   This creates a scaffolded change at \`lightspec/changes/<name>/\` with the selected schema.

4. **Show the artifact status**
   \`\`\`bash
   lightspec status --change "<name>"
   \`\`\`
...
5. **Get instructions for the first artifact**
   ...
   \`\`\`bash
   lightspec instructions <first-artifact-id> --change "<name>"
   \`\`\`
...
/**
 * Template for lightspec-apply-change skill
...
1. **Select the change**
   ...
   - If ambiguous, run \`lightspec list --json\` to get available changes and use the **AskUserQuestion tool** to let the user select

   Always announce: "Using change: <name>" and how to override (e.g., \`/opsx:apply <other>\`).

2. **Check status to understand the schema**
   \`\`\`bash
   lightspec status --change "<name>" --json
   \`\`\`
...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If \`state: "blocked"\` (missing artifacts): show message, suggest using lightspec-continue-change
...
/**
 * Template for lightspec-ff-change skill
...
2. **Create the change directory**
   ...
   lightspec new change "<name>"
   \`\`\`
   This creates a scaffolded change at \`lightspec/changes/<name>/\`.
...
4. **Create artifacts in sequence until apply-ready**
   ...
   a. **For each artifact that is \`ready\` (dependencies satisfied)**:
      - Get instructions:
        \`\`\`bash
        lightspec instructions <artifact-id> --change "<name>" --json
        \`\`\`
   ...
   b. **Continue until all \`applyRequires\` artifacts are complete**
      - After creating each artifact, re-run \`lightspec status --change "<name>" --json\`
...
- Follow the \`instruction\` field from \`lightspec instructions\` for each artifact type
...
/**
 * Template for lightspec-sync-specs skill
...
1. **If no change name provided, prompt for selection**
   ...
   Run \`lightspec list --json\` to get available changes. Use the **AskUserQuestion tool** to let the user select.
...
2. **Find delta specs**
   ...
   Look for delta spec files in \`lightspec/changes/<name>/specs/*/spec.md\`.
...
3. **For each delta spec, apply changes to main specs**
   ...
   For each capability with a delta spec at \`lightspec/changes/<name>/specs/<capability>/spec.md\`:

   a. **Read the delta spec** to understand the intended changes

   b. **Read the main spec** at \`lightspec/specs/<capability>/spec.md\` (may not exist yet)
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      - Create \`lightspec/specs/<capability>/spec.md\`
...
**Input**: The argument after \`/lightspec:explore\` is whatever the user wants to think about. Could be:
...
1. **If no change name provided, prompt for selection**
   ...
   Run \`lightspec list --json\` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.
...
export function getOpsxApplyCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: Apply',
    description: 'Implement tasks from an LightSpec change (Experimental)',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'experimental'],
    content: `Implement tasks from an LightSpec change.
...
/**
 * Template for lightspec-archive-change skill
...
2. **Check artifact completion status**
   ...
   Run \`lightspec status --change "<name>" --json\` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   Check for delta specs at \`lightspec/changes/<name>/specs/\`. If none exist, proceed without sync prompt.

   **If delta specs exist:**
   - Compare each delta spec with its corresponding main spec at \`lightspec/specs/<capability>/spec.md\`
   ...
   **Prompt options:**
   ...
   If user chooses sync, execute /opsx:sync logic (use the lightspec-sync-specs skill). Proceed to archive regardless of choice.

5. **Perform the archive**

   Create the archive directory if it doesn't exist:
   \`\`\`bash
   mkdir -p lightspec/changes/archive
   \`\`\`
   ...
   **Check if target already exists:**
   ...
   \`\`\`bash
   mv lightspec/changes/<name> lightspec/changes/archive/YYYY-MM-DD-<name>
   \`\`\`
...
**Change:** <change-name>
**Schema:** <schema-name>
**Archived to:** lightspec/changes/archive/YYYY-MM-DD-<name>/
...
- Use artifact graph (lightspec status --json) for completion checking
- Don't block archive on warnings - just inform and confirm
- Preserve .lightspec.yaml when moving to archive (it moves with the directory)
- Show clear summary of what happened
- If sync is requested, use lightspec-sync-specs approach (agent-driven)
- If delta specs exist, always run the sync assessment and show the combined summary before prompting`
  };
}

/**
 * Template for lightspec-bulk-archive-change skill
...
1. **Get active changes**
   ...
   Run \`lightspec list --json\` to get all active changes.
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run \`lightspec status --change "<name>" --json\`
      - Parse \`schemaName\` and \`artifacts\` list
      - Note which artifacts are \`done\` vs other states

   b. **Task completion** - Read \`lightspec/changes/<name>/tasks.md\`
      - Count \`- [ ]\` (incomplete) vs \`- [x]\` (complete)
      - If no tasks file exists, note as "No tasks"

   c. **Delta specs** - Check \`lightspec/changes/<name>/specs/\` directory
...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync specs** if delta specs exist:
      - Use the lightspec-sync-specs approach (agent-driven intelligent merge)
...
- Preserve .lightspec.yaml when moving to archive
...
/**
 * Template for lightspec-verify-change skill
...
5. **Verify Completeness**
   ...
   **Spec Coverage**:
   - If delta specs exist in \`lightspec/changes/<name>/specs/\`:
...
**Change:** <change-name>
**Target:** lightspec/changes/archive/YYYY-MM-DD-<name>/
...
export function getFeedbackSkillTemplate(): SkillTemplate {
  return {
    name: 'feedback',
    description: 'Collect and submit user feedback about LightSpec with context enrichment and anonymization.',
    instructions: `Help the user submit feedback about LightSpec.
...
5. **Submit on confirmation**
   - Use the \`lightspec feedback\` command to submit
   - Format: \`lightspec feedback "title" --body "body content"\`
