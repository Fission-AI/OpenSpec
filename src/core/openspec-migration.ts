/**
 * Legacy Tool → Pastelsdd Migration
 *
...
/**
 * Runs the legacy tool → pastelsdd migration.
...
export async function runLegacyToolMigration(
  ...
  if (detection.hasLegacyDir) {
    ...
    if (pastelsddExists) {
      ...
      try {
        await fs.rename(detection.legacyDirPath, pastelsddPath);
        result.renamedDir = true;
      } catch (err: any) {
        result.errors.push(`Could not rename ${detection.legacyDirName}/ → pastelsdd/: ${err.message}`);
      }
    }
  }
