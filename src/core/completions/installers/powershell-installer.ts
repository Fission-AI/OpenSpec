...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'ClearSpecCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add ClearSpec completion configuration with markers
        const clearspecBlock = [
          '',
        ...
        const newContent = profileContent + clearspecBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Remove CLEARSPEC:START -> CLEARSPEC:END block
        const startMarker = '# CLEARSPEC:START';
        const endMarker = '# CLEARSPEC:END';
        const startIndex = profileContent.indexOf(startMarker);

        if (startIndex === -1) {
          continue; // No ClearSpec block found
        }
