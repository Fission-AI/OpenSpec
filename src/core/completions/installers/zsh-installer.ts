...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_pastelsdd'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_pastelsdd'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  async configureZshrc(completionsDir: string): Promise<boolean> {
    ...
    if (process.env.PASTELSDD_NO_AUTO_CONFIG === '1') {
      return false;
    }
  ...
  /**
   * Check if .zshrc has Pastelsdd configuration markers
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed Pastelsdd configuration from ~/.zshrc');
      }
