...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_lightspec'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_lightspec'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  /**
   * Check if .zshrc has LightSpec configuration markers
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed LightSpec configuration from ~/.zshrc');
      }
