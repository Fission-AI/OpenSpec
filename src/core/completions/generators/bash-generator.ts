...
/**
 * Generates Bash completion scripts for the ClearSpec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for ClearSpec CLI
...
${helpers}
complete -F _clearspec_completion clearspec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_clearspec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_clearspec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_clearspec_complete_items`);
        break;
      case 'schema-name':
        lines.push(`${indent}_clearspec_complete_schemas`);
        break;
      case 'shell':
