...
# Use pastelsdd __complete to get available changes
_pastelsdd_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(pastelsdd __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use pastelsdd __complete to get available specs
_pastelsdd_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(pastelsdd __complete specs 2>/dev/null)
...
# Use pastelsdd __complete to get available schemas
_pastelsdd_complete_schemas() {
  local -a schemas
  while IFS=$'\\t' read -r id desc; do
    schemas+=("$id:$desc")
  done < <(pastelsdd __complete schemas 2>/dev/null)
