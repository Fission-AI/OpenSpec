 ...
 * - Helps understand ClearSpec's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other ClearSpec schemas
 */
...
/**
 * Read and parse clearspec/config.yaml from project root.
 ...
 *
 * @param projectRoot - The root directory of the project (where `clearspec/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, CLEARSPEC_DIR_NAME, 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, CLEARSPEC_DIR_NAME, 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`clearspec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse clearspec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in clearspec/config.yaml\n\n`;
  ...
  message += `\nFix: Edit clearspec/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
