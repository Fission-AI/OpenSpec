 ...
 * - Helps understand LightSpec's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other LightSpec schemas
 */
...
/**
 * Read and parse lightspec/config.yaml from project root.
 ...
 *
 * @param projectRoot - The root directory of the project (where `lightspec/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, 'lightspec', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'lightspec', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`lightspec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse lightspec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in lightspec/config.yaml\n\n`;
  ...
  message += `\nFix: Edit lightspec/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
