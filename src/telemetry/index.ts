 ...
 * - Opt-out via CLEARSPEC_TELEMETRY=0 or DO_NOT_TRACK=1
...
const POSTHOG_HOST = 'https://edge.clearspec.dev';
...
export function isTelemetryEnabled(): boolean {
  ...
  if (process.env.CLEARSPEC_TELEMETRY === '0') {
    return false;
  }
 ...
 * @param version - The ClearSpec version
 */
...
export async function maybeShowTelemetryNotice(): Promise<void> {
  ...
  try {
    ...
    // Display notice
    console.log(
      'Note: ClearSpec collects anonymous usage stats. Opt out: CLEARSPEC_TELEMETRY=0'
    );
