...
| `anvil` | @jikkujoyce | [jikkujoyce/openspec-schemas](https://github.com/jikkujoyce/openspec-schemas/tree/main/schemas/anvil) | Spec-driven workflow with TDD discipline and an adversarial review step. Flow: `proposal` → `specs` → `design` → `review` → `test-plan` → `tasks` → `apply` → `verify`. `review` is written by a fresh-context, read-only reviewer (a second model when one is available) and emits a machine-readable `VERDICT:` line instructing the agent to gate `test-plan`, `tasks`, and `apply`; a verdict is void once a reviewed artifact changes. The gate is advisory — OpenSpec `requires:` only checks that artifact files exist, so enforce it with your own CI or hook. `test-plan` maps every spec scenario 1:1 to a named test and doubles as a red/green ledger that `verify` audits. |
