...
| `anvil` | @jikkujoyce | [jikkujoyce/openspec-schemas](https://github.com/jikkujoyce/openspec-schemas/tree/main/schemas/anvil) | Spec-driven workflow with TDD discipline and an adversarial review gate. Flow: proposal → specs → design → review → test-plan → tasks → apply → verify. The `review` artifact is produced by a fresh-context, read-only reviewer (a second model when one is available) and emits a machine-readable `VERDICT:` line that gates test-plan, tasks, and apply — editing a reviewed artifact voids the verdict. `test-plan` maps every spec scenario 1:1 to a named test and doubles as a live red/green ledger that `verify` audits. |
