...
This document describes how users can customize LightSpec schemas and templates, the current manual process, and the gap that needs to be addressed.

---

## Overview

LightSpec uses a 2-level schema resolution system following the XDG Base Directory Specification:

1. **User override**: `${XDG_DATA_HOME}/lightspec/schemas/<name>/`
...
| All (if set) | `$XDG_DATA_HOME/lightspec/schemas/` |

### 2. Create the directory structure

```bash
# macOS/Linux example
mkdir -p ~/.local/share/lightspec/schemas/spec-driven/templates
```
...
# or
which lightspec && readlink -f $(which lightspec)

# Copy files from the package's schemas/ directory
cp <package-path>/schemas/spec-driven/schema.yaml ~/.local/share/lightspec/schemas/spec-driven/
cp <package-path>/schemas/spec-driven/templates/*.md ~/.local/share/lightspec/schemas/spec-driven/templates/
```
...
| **No diffing** | When upgrading lightspec, users can't see what changed in built-in templates |
...
5. *"I upgraded lightspec, did the templates change?"* — no way to diff
...
### Option A: Single `lightspec schema` command
...
# Output: /Users/me/.local/share/lightspec/schemas/spec-driven/ (user override)
# Output: /usr/local/lib/node_modules/lightspec/schemas/spec-driven/ (built-in)

# Copy a built-in schema to user directory for customization
lightspec schema copy spec-driven
# Creates ~/.local/share/lightspec/schemas/spec-driven/ with all files
...
# Validate a schema
lightspec schema validate spec-driven
```

### Option B: Dedicated `lightspec customize` command
...
lightspec customize spec-driven
...
1. `lightspec schema list` — see what's available
2. `lightspec schema which <name>` — debug resolution
3. `lightspec schema copy <name>` — scaffold customization
4. `lightspec schema diff <name>` — compare with built-in
5. `lightspec schema reset <name>` — revert to defaults
