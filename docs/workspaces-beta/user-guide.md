...
ClearSpec asks for the context store name, where to put it, and whether to
...
> Create a ClearSpec initiative called `billing-launch` in `team-context`. Keep
> it short and useful.
...
Select the initiative from the picker. ClearSpec creates a local workspace view
...
when attached, and a small `ClearSpec workspace` folder last with `AGENTS.md`,
`.clearspec-workspace/view.yaml`, and the generated `.code-workspace` file.

Use `clearspec workspace open --initiative team-context/billing-launch --editor`
...
> Check this ClearSpec workspace. Resolve the selected initiative, list the
...
ClearSpec does not clone anything.
...
> ClearSpec artifacts should live.
...
> Create a draft repo-local ClearSpec proposal for the owning linked repo and
> link it to `team-context/billing-launch`. Resolve the workspace and initiative
> context yourself, run the needed ClearSpec commands from the correct repo, and
> report the files you created.

## Tiny Caveat Box

ClearSpec is not cloning, syncing, branching, or tracking progress dashboards in
