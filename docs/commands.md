...
This is the reference for ClearSpec's slash commands. These commands are invoked in your AI coding assistant's chat interface (e.g., Claude Code, Cursor, Windsurf).
...
| `/clsx:apply` | Implement tasks from the change |
| `/clsx:sync` | Merge delta specs into main specs |
| `/clsx:archive` | Archive a completed change |
...
| `/clsx:continue` | Create the next artifact based on dependencies |
| `/clsx:ff` | Fast-forward: create all planning artifacts at once |
| `/clsx:verify` | Validate implementation matches artifacts |
| `/clsx:bulk-archive` | Archive multiple changes at once |
| `/clsx:onboard` | Guided tutorial through the complete workflow |

The default global profile is `core`. To enable expanded workflow commands, run `clearspec config profile`, select workflows, then run `clearspec update` in your project.
...
**Syntax:**
```text
/clsx:propose [change-name-or-description]
```
...
AI:  Created clearspec/changes/add-dark-mode/
     ...
     ✓ design.md
     ✓ tasks.md
     Ready for implementation. Run /clsx:apply.
```

**Tips:**
- Use this for the fastest end-to-end path
- If you want step-by-step artifact control, enable expanded workflows and use `/clsx:new` + `/clsx:continue`
...
- Can transition to `/clsx:propose` (default) or `/clsx:new` (expanded workflow) when insights crystallize
...
AI:  Ready when you are. Run /clsx:propose add-jwt-auth to begin.
```
...
Start a new change scaffold. Creates the change folder and waits for you to generate artifacts with `/clsx:continue` or `/clsx:ff`.

This command is part of the expanded workflow set (not included in the default `core` profile).

**Syntax:**
```
/clsx:new [change-name] [--schema <schema-name>]
```
...
**What it does:**
- Creates `clearspec/changes/<change-name>/` directory
- Creates `.clearspec.yaml` metadata file in the change folder
- Shows the first artifact template ready for creation
- Prompts for change name and schema if not provided

**What it creates:**
```
clearspec/changes/<change-name>/
└── .clearspec.yaml    # Change metadata (schema, created date)
```

**Example:**
```
You: /clsx:new add-dark-mode

AI:  Created clearspec/changes/add-dark-mode/
     Schema: spec-driven

     Ready to create: proposal
     Use /clsx:continue to create it, or /clsx:ff to create all artifacts.
```

**Tips:**
- Use descriptive names: `add-feature`, `fix-bug`, `refactor-module`
- Avoid generic names like `update`, `changes`, `wip`
- Schema can also be set in project config (`clearspec/config.yaml`)
...
AI:  Change: add-dark-mode
     ...
     ✓ Created clearspec/changes/add-dark-mode/specs/ui/spec.md

     Now available: tasks
     Run /clsx:continue to create the next artifact.
```
...
AI:  Fast-forwarding add-dark-mode...
     ...
     Ready for implementation. Run /clsx:apply to begin.
```

**Tips:**
- Use when you have a clear picture of what you're building
- Faster than `/clsx:continue` for straightforward changes
...
- Merges changes into main `clearspec/specs/` directory
...
AI:  Syncing add-dark-mode delta specs...
     ...
     Reading: clearspec/changes/add-dark-mode/specs/ui/spec.md
     Target:  clearspec/specs/ui/spec.md
     ...
     Merging...
     ✓ clearspec/specs/ui/spec.md updated

     Change remains active. Run /clsx:archive when ready to complete.
```
...
- Moves change folder to `clearspec/changes/archive/YYYY-MM-DD-<name>/`
...
AI:  ✓ Synced specs to clearspec/specs/ui/spec.md
     ✓ Moved to clearspec/changes/archive/2025-01-24-add-dark-mode/
...
- Use `/clsx:verify` first to catch issues
...
**Syntax:**
```
/clsx:bulk-archive [change-names...]
```
...
Guided onboarding through the complete ClearSpec workflow. An interactive tutorial using your actual codebase.
...
3. Creating a change (`/clsx:new`)
4. Writing the proposal
5. Creating specs
6. Writing the design
7. Creating tasks
8. Implementing tasks (`/clsx:apply`)
...
| Tool | Syntax Example |
|------|----------------|
| Claude Code | `/clsx:propose`, `/clsx:apply` |
| Cursor | `/clsx-propose`, `/clsx-apply` |
| Windsurf | `/clsx-propose`, `/clsx-apply` |
| Copilot (IDE) | `/clsx-propose`, `/clsx-apply` |
| Kimi CLI | Skill-based invocations such as `/skill:clearspec-propose`, `/skill:clearspec-apply-change` (no generated `clsx-*` command files) |
| Trae | Skill-based invocations such as `/clearspec-propose`, `/clearspec-apply-change` (no generated `clsx-*` command files) |
...
|---------|--------------|
| `/clearspec:proposal` | Create all artifacts at once (proposal, specs, design, tasks) |
| `/clearspec:apply` | Implement the change |
| `/clearspec:archive` | Archive the change |
...
**Solutions:**
- Specify the change name explicitly: `/clsx:apply add-dark-mode`
- Check that the change folder exists: `clearspec list`
...
**Solutions:**
- Run `clearspec status --change <name>` to see what's blocking
...
**Solutions:**
- List available schemas: `clearspec schemas`
- Check spelling of schema name
- Create the schema if it's custom: `clearspec schema init <name>`

### Commands not recognized

The AI tool doesn't recognize ClearSpec commands.

**Solutions:**
- Ensure ClearSpec is initialized: `clearspec init`
- Regenerate skills: `clearspec update`
...
**Solutions:**
- Add project context in `clearspec/config.yaml`
- Add per-artifact rules for specific guidance
- Provide more detail in your change description
- Use `/clsx:continue` instead of `/clsx:ff` for more control
