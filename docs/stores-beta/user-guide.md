...
- **Schema inspection follows the consumer config.** `schemas` follows the
  canonical root-selection precedence and accepts `--store <id>`; `templates`
  resolves from the nearest consumer project and does not accept `--store`.
  Both honor that consumer's `schemaStore`, and `schemas --json` keeps its
  successful array shape unchanged. Deprecated noun forms such as
  `openspec change show` remain current-directory commands.
