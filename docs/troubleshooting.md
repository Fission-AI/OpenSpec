...
6. **Confirm your tool supports command files.** Codex, CodeArts, ForgeCode, Hermes, Kimi Code, MiniMax Code, Mistral Vibe, Rovo Dev CLI, Warp, Zed Agent, and the shared `.agents` target don't get generated `opsx-*` command files; they use skills instead, so `/opsx` will never autocomplete for them. Type `$openspec-propose` in Codex, `/skill:openspec-propose` in Kimi Code, and `/openspec-propose` in the other slash-capable tools. Rovo Dev CLI has no slash-command surface, so ask it to use `openspec-propose` by name. The shared `.agents` target is vendor-neutral, so `/openspec-propose` is the common form rather than a guaranteed one. If your assistant does not answer to it, check its own docs for how it invokes a skill. Amazon Q does get command files, but loads them into its prompt library rather than its slash menu, so type `@opsx-propose` there, not `/opsx`. Every tool's form is listed in [How To Invoke](supported-tools.md#how-to-invoke).
