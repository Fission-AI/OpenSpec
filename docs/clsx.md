...
OPSX is now the standard workflow for ClearSpec.

It's a **fluid, iterative workflow** for ClearSpec changes. No more rigid phases — just actions you can take anytime.

## Why This Exists

The legacy ClearSpec workflow works, but it's **locked down**:
...
- **ClearSpec contributors** — experiment with new approaches without releases
...
```bash
# Make sure you have clearspec installed — skills are automatically generated
clearspec init
```

This creates skills in `.claude/skills/` (or equivalent) that AI coding assistants auto-detect.

By default, ClearSpec uses the `core` workflow profile (`propose`, `explore`, `apply`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `bulk-archive`, `onboard`), configure them with `clearspec config profile` and apply with `clearspec update`.

During setup, you'll be prompted to create a **project config** (`clearspec/config.yaml`). This is optional but recommended.
...
Config is created during `clearspec init`, or manually:

```yaml
# clearspec/config.yaml
schema: spec-driven
...
2. Change metadata (`.clearspec.yaml` in change directory)
...
- Run `clearspec schemas --json` to see artifact IDs for each schema

**Config not being applied:**
- Ensure file is at `clearspec/config.yaml` (not `.yml`)
...
|---------|--------------|
| `/clsx:propose` | Create a change and generate planning artifacts in one step (default quick path) |
| `/clsx:explore` | Think through ideas, investigate problems, clarify requirements |
| `/clsx:new` | Start a new change scaffold (expanded workflow) |
| `/clsx:continue` | Create the next artifact (expanded workflow) |
| `/clsx:ff` | Fast-forward planning artifacts (expanded workflow) |
| `/clsx:apply` | Implement tasks, updating artifacts as needed |
| `/clsx:verify` | Validate implementation against artifacts (expanded workflow) |
| `/clsx:sync` | Sync delta specs to main (default workflow, optional) |
| `/clsx:archive` | Archive when done |
| `/clsx:bulk-archive` | Archive multiple completed changes (expanded workflow) |
| `/clsx:onboard` | Guided walkthrough of an end-to-end change (expanded workflow) |
...
/clsx:explore
```
Think through ideas, investigate problems, compare options. No structure required - just a thinking partner. When insights crystallize, transition to `/clsx:propose` (default) or `/clsx:new`/`/clsx:ff` (expanded).
...
```text
/clsx:new        # scaffold only
/clsx:continue   # create one artifact at a time
/clsx:ff         # create all planning artifacts at once
```
...
/clsx:apply
```
Works through tasks, checking them off as you go. If you're juggling multiple changes, you can run `/clsx:apply <name>`; otherwise it should infer from the conversation and prompt you to choose if it can't tell.

### Finish up
```
/clsx:archive   # Move to archive when done (prompts to sync specs if needed)
```
...
| | Legacy (`/clearspec:proposal`) | OPSX (`/clsx:*`) |
|---|---|---|
...
│         │                     │                     │                       │
│         ▼                     ▼                     ▼                       │
│   /clearspec:proposal   /clearspec:apply      /clearspec:archive              │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Generated Command Files (.claude/commands/clearspec/*.md)                 │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Skill Files (.claude/skills/clearspec-*/SKILL.md)                          │
│                                                                             │
...
```
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ clearspec status --change "add-auth" --json                      │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
  │  │    "artifacts": [                                                  │  │
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ clearspec instructions specs --change "add-auth" --json          │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
...
```
  /clsx:new ───► /clsx:continue ───► /clsx:apply ───► /clsx:archive
      │                │                  │
      │                │                  ├── "The design is wrong"
      │                │                  │
      │                │                  ▼
...
clearspec schema which my-workflow
```

Schemas are stored in `clearspec/schemas/` (project-local, version controlled) or `~/.local/share/clearspec/schemas/` (user global).

**Schema structure:**
```
clearspec/schemas/research-first/
├── schema.yaml
└── templates/
    ├── research.md
    ├── proposal.md
...
- Use `/clsx:explore` to think through an idea before committing to a change
- `/clsx:ff` when you know what you want, `/clsx:continue` when exploring
- During `/clsx:apply`, if something's wrong — fix the artifact, then continue
- Tasks track progress via checkboxes in `tasks.md`
- Check status anytime: `clearspec status --change "name"`

## Feedback

This is rough. That's intentional — we're learning what works.

Found a bug? Have ideas? Join us on [Discord](https://discord.gg/YctCnvvshC) or open an issue on [GitHub](https://github.com/<you>/clearspec/issues).
