...
This guide explains how ClearSpec works after you've installed and initialized it. For installation instructions, see the [main README](../README.md#quick-start).

## How It Works

ClearSpec helps you and your AI coding assistant agree on what to build before any code is written.
...
```text
/clsx:new ──► /clsx:ff or /clsx:continue ──► /clsx:apply ──► /clsx:verify ──► /clsx:archive
```

The default global profile is `core`, which includes `propose`, `explore`, `apply`, `sync`, and `archive`. You can enable the expanded workflow commands with `clearspec config profile` and then `clearspec update`.

## What ClearSpec Creates

After running `clearspec init`, your project has this structure:
...
Delta specs are the key concept in ClearSpec. They show what's changing relative to your current specs.
...
The change folder moves to `clearspec/changes/archive/` for audit history.
...
If you've enabled the expanded workflow profile, you can also do this as two steps: `/clsx:new` then `/clsx:ff` (or `/clsx:continue` incrementally).
...
AI:  Archiving add-dark-mode...
     ✓ Merged specs into clearspec/specs/ui/spec.md
...
- [Customization](customization.md) - Make ClearSpec work your way
