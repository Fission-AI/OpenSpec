...
run `openspec completion install`. (The scripts are generated by running the binary the
build just produced, so a cross-compiled build omits them; use
`openspec completion install` there.)
