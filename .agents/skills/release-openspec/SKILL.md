...
1. If an open changeset PR already covers the missing work, reuse it. Do not create a duplicate.
2. Read `.changeset/README.md` immediately before authoring.
3. Branch from current `origin/main`. Use a short `changeset-<scope>` branch and a temporary worktree when needed.
...
2. Confirm it consumes every pending changeset selected for this release.
3. Confirm `package.json`, `CHANGELOG.md`, the PR body, and the proposed version agree.
4. Confirm the version is the correct highest bump from the selected changesets.
5. Confirm it is based on current `main` and no newly merged user-facing work needs a release decision.
6. Run or inspect `pnpm exec changeset status` as a consistency check.
7. Inspect all required checks and review state with `gh pr view` / `gh pr checks`.
