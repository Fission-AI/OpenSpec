...
8. **Execute archive for each confirmed change**
   ...
   b. **Verify included delta specs before moving changeRoot**:
      ...
      - Verify that main specs are updated:
        ...
        - REMOVED requirements gone — and where this sync retired a capability (removed its last requirement, leaving `## Requirements` empty), its main spec moved into the change's `retired-specs/` rather than left empty; a spec the sync deliberately kept and reported is also a match
